 ...
 */
PASCAL NEAR setfillcol( int f, int n)
 ...
 */
PASCAL NEAR showcpos(int f, int n)

{
	...
	curwp->w_doto = savepos;
        ...
        if (numchars != 0)
                ratio = (100 * (int)predchars) / (int)numchars;
...
PASCAL NEAR getlinenum(	/* get the a line number */
...

{
        ...
        if (!stabsize)
                return(linsert(1, '\t', TRUE));
        return(linsert(stabsize - (getccol(FALSE) % stabsize), ' ', TRUE));
}

PASCAL NEAR detab(	/* change tabs to spaces */
...
PASCAL NEAR entab(	/* change spaces to tabs where posible */
...

{
	...
	register LINE *lp;	/* line loop */

	if (curbp->b_mode&MDVIEW)	/* don't allow this command if	*/
		return(rdonly());	/* we are in read only mode	*/

	/* folded buffers cannot be safely entabbed, due to
	 * tabs crossing left margines etc. so don't allow it!
	 */
			...
			if ((fspace >= 0) && (nextab(fspace) <= ccol))
			  {
				if (ccol - fspace < 2)
					...
					ldelete(((long)ccol - fspace), FALSE,FALSE,FALSE);
					linsert(1, '\t', FALSE);	
					fspace = -1;
				}
			      }
...

{
	...
	lp = curwp->w_dotp;
	offset = curwp->w_doto;
	i = offset - curwp->w_dotp->l_lmargin; /* only goto margin */
	...
	lp = curwp->w_dotp;
	if ((lp->l_used != 0) && (lp->l_used != lp->l_lmargin))
		return(TRUE);

	/* hunt for the last non-blank line to get indentation from */
	while (((lp->l_used == 0) || (lp->l_used == lp->l_lmargin)) &&
	       (lp != curbp->b_linep))
		lp = lp->l_bp;

	/* grab a pointer to text to copy indentation from */
	cptr = &(lp->l_text[lp->l_lmargin]);
...
PASCAL NEAR insbrace(	/* insert a brace into the text here...we are in CMODE */
	...
	while (target != getccol(FALSE)) {
		if (target < getccol(FALSE))	/* on doit detruire des caracteres */
			...
			while (target - getccol(FALSE) >= tabsize)
				linsert(1,'\t',TRUE);
			linsert(target - getccol(FALSE), ' ',TRUE);
		}
	}
...

{
	...
	/* delete back first */
	while (getccol(FALSE) > curwp->w_dotp->l_lmargin)
		backdel(FALSE, 1);
...

{
	...
	if (curwp->w_doto < curwp->w_dotp->l_lmargin)
        ...
        return(ldelete(chunk, TRUE,FALSE,FALSE));
}

PASCAL NEAR setmod(	/* prompt and set an editor mode */

  int f,
  int n )	/* default and argument */

{
	return(adjustmode(TRUE, FALSE));
}

PASCAL NEAR delmode(	/* prompt and delete an editor mode */
...
PASCAL NEAR setgmode(	/* prompt and set a global editor mode */
...
PASCAL NEAR delgmode(	/* prompt and delete a global editor mode */
...
PASCAL NEAR adjustmode(	/* change the editor mode status */
...
PASCAL NEAR istring(	/* ask for and insert a string into the current
		   buffer at the current point */

  int f,
  int n )	/* ignored arguments */
...
PASCAL NEAR ovstring( /* ask for and overwite a string into the current
		   buffer at the current point */
