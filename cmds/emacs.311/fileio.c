...
{
#ifdef __HELIOS
	{ Object	*tmp = Locate(cdobj(), fn);
	  if (tmp == NULL) return(FIOFNF);
	  if ((tmp->Type & Type_Flags) != Type_Stream)
	   { Close(tmp); return(FIODIR); }
	  Close(tmp);
	}
#endif
...
#if     V7 | USG | SMOS | HPUX | SUN | XENIX | BSD | WMCS | VMS | (MSDOS & (LATTICE | MSC | DTL | TURBO | ZTC)) | OS2 | (TOS & MWC) | AVIION | HELIOS
        if (fclose(ffp) != FALSE) {
                mlwrite(TEXT156);
/*                      "Error closing file" */
                return(FIOERR);
...
int PASCAL NEAR fexist(	/* does <fname> exist on disk? */
