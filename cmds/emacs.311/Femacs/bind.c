...
PASCAL NEAR betawarning(f, n)	/* warn them !!!!
		   bring up a fake buffer and read the beta warning file
		   into it with view mode			*/
{
	...
	bp = bfind("emacs.inf", FALSE, BFINVS);

	if (bp == NULL) {
#if SHARED
	        strcpy(tname, "emacs.inf");
        	fname = flook(tname, FALSE);
#else       
		fname = flook("emacs.inf", FALSE);
#endif
		if (fname == NULL) {
			mlwrite(TEXT12);
