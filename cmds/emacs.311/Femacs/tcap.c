/*	tcap:	Unix V5, SUN OS, SCO XENIX, V7 and BS4.2 Termcap video driver
 ...
 li|ansi|Ansi standard crt:\
 	...
 	o  Andy Sturges - Inmos : Fixed function key handling for
 	   SunOS V.4 so it does not rely on select as much for catching function
 	   keys.
...
#endif
	char cseq[MAX_CSEQ];		/* current sequence being parsed */
	if (meta_func) {
		c = meta_func;
		meta_func = 0;
		return c;
...
#endif
		{
			/* Check for M-Function key */
			if ((c = ttgetc()) == 27) {
				meta_esc = 1;
				c = ttgetc();
			}
	  		while (sp < &cseq[MAX_CSEQ] && match) {
				/* check to see if things are pending soon */
		  		*sp++ = c; l++;
				match = 0;
		  		for (index = 0; index < NTBINDS; index++) {
					if (strncmp(cseq, ttable[index].p_seq, l) == 0) {
						match = 1;
						if (ttable[index].p_seq[l] == '\0') {
							if (meta_esc) {
								meta_func = ttable[index].p_code;
								return CTRL | '[';
							} else
				...
				if (sp < &cseq[MAX_CSEQ] && match) c = ttgetc();
			}
	        	*sp = 0;
			sp = &cseq[1] ;
			while (*sp) in_put(*(sp++));
