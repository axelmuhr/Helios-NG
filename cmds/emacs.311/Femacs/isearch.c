...
start_over:
    ...
    col = promptpattern(TEXT165);		/* Prompt, remember the col   */
/*                      "ISearch: " */
        ...
        (c == IS_VMSFORW))			/* Reuse old search string?   */
    {
    	for (cpos = 0; pat[cpos] != 0; cpos++)	/* Yup, find the length	      */
    	    col = echochar((int)pat[cpos],col);	/*  and re-echo the string    */
...
int PASCAL NEAR checknext(chr, patrn, dir) /* Check next character in search string */
...
int PASCAL NEAR scanmore(patrn, dir)	/* search forward or back for a pattern	      */
char	*patrn;			/* string to scan for			      */
int	dir;			/* direction to search			      */
{
	int	sts;			/* search status		      */

	setjtable(patrn);		/* Set up fast search arrays  */
...
int PASCAL NEAR match_pat (patrn) /* See if the pattern string matches string at "."   */
...

{
	...
	switch (c)				/* Yes, dispatch special cases*/
	{
	  ...
	  case 0x7F:				/* Rubout:		      */
	    mlout('^');		/* Output a funny looking     */
	    mlout('?');		/*  indication of Rubout      */
