...
static int enterfolds;	/* MJB: 21-Sep-89 */
...
/* Function prototypes */
#if PROTO
int PASCAL NEAR fsearch(int, int);
int PASCAL NEAR bksearch(int, int);
#else
int PASCAL NEAR fsearch();
int PASCAL NEAR bksearch();
#endif

/*
 * searchffolds -- Search forwards into folds, open those folds
 * 		   required to display the match. MJB: 21-Sep-89.
 */
PASCAL NEAR searchffold(
  int f,
...
/*
 * forwsearch -- Search forwards ignore folds. MJB: 21-Sep-89.
 */
...
/*
 * searchbfolds -- Search backwards into folds, open those folds
 * 		   required to display the match. MJB: 21-Sep-89.
 */
PASCAL NEAR searchbfold(
  int f,
  int n )	/* default flag / numeric argument */
{
	enterfolds = TRUE;
...
/*
 * backsearch -- Search backwards ignore folds. MJB: 21-Sep-89.
 */
...
/*
 * fsearch -- Search forward.  Get a search string from the user, and
...
/*
 * bksearch -- Reverse search.  Get a search string from the user, and
...
int		*pcwoff )	/* position within current line */
{
	...
	while (mcptr->mc_type != MCNIL) {
		...
		if (cl_type = (mcptr->mc_type & ALLCLOS)) {
			...
			group_reg[mcptr->u.group_no].r_size = - (long) matchlen;
		}
...
int PASCAL NEAR savematch()
{
	...
	if ((patmatch = (char *)malloc(matchlen + 1)) == NULL) {
		mlwrite(TEXT94);
/*			"%%Out of memory" */
		return ABORT;
	}
	...
	for (j = 1; j <= group_count; j++) {
		...
		if ((grpmatch[j] = (char *)malloc((int) group_reg[j].r_size + 1)) != NULL) {
...
/*				"with" */
	...
	/* Make sure we go down into folds. MJB: 13-Oct-89 */
	enterfolds = TRUE;
...
/*"(Y)es, (N)o, (!)Do rest, (U)ndo last, (^G)Abort, (.)Abort back, (?)Help: "*/
		...
		if (kind) {
			...
			if ((oldpatmatch = (char *)malloc(matchlen + 1)) == NULL) {
				mlwrite(TEXT94);
/*					"%%Out of memory" */
				return ABORT;
			}
...
int	use_rmc )
{
	 ...
	 */
	if ((status = ldelete((long) dlength, FALSE, FALSE, FALSE)) != TRUE)
		mlwrite(TEXT93);
		...
		switch (*patptr) {
			case MC_DITTO:
				...
				if (mj != 0) {
					...
					if ((rmcptr->u.rstr = (char *)malloc(mj + 1)) == NULL) {
						mlwrite(TEXT94);
/*							"%%Out of memory" */
						status = FALSE;
						break;
...
/*							"%%Out of memory" */
				...
				else
				{
					 ...
					 * current character.
					 */
					if ((rmcptr->u.rstr = (char *)malloc(mj + 2)) == NULL) {
						mlwrite(TEXT94);
/*						"%%Out of memory" */
						status = FALSE;
						break;
