...
/*	Modifications:
	29-Sep-89	Mike Burrow (INMOS)	Added folding.
*/
...
char *PASCAL NEAR gtfun(	/* evaluate a function */
...
char *PASCAL NEAR gtusr(	/* look up a user var's value */
...
char *PASCAL NEAR funval(int i)

{
	return(funcs[i].f_name);
}

char *PASCAL NEAR envval(int i )
...
/* Convert line type into a string */
char *PASCAL NEAR ltypetos(int ltype )
{
	switch(ltype) {
		case LEOEFOLD : return("LEOEFOLD");
		case LSOEFOLD : return("LSOEFOLD");
		case LEOFOLD  : return("LEOFOLD");
		case LSOFOLD  : return("LSOFOLD");
		default	      : return("LNORMAL");
	}
}
...

{
	...
	switch (vnum) {
		...
		case EVLTYPE:	return(ltypetos(curwp->w_dotp->l_type));
		case EVLMARGIN: return(int_asc(curwp->w_dotp->l_lmargin));
		case EVZPOS:	return(int_asc((int)curbp->b_nfolds));
		default:
		case EVBACKUP:  return(ltos(backup_flag));
	}
	meexit(-12);	/* again, we should never get here */
}

char *PASCAL NEAR fixnull(	/* Don't return NULL pointers! */
...
char *PASCAL NEAR trimstr(	/* trim whitespace off the end of a string */
...
int PASCAL NEAR setvar(		/* set a variable */
...
PASCAL NEAR findvar(	/* find a variables type and name */
...
int PASCAL NEAR svar(	/* set a variable */
...

{
	...
	case TKENV: /* set an environment variable */
		...
		case EVLASTKEY: lastkey = asc_int(value);
				break;
		case EVCURCHAR: ldelete(1L, FALSE,FALSE,TRUE);	/* delete 1 char */
				c = asc_int(value);
				if (c == '\r')
					lnewline();
				else
					linsert(1, (char)c, TRUE);
				backchar(FALSE, 1);
				break;
		...
		case EVYANKFLAG:yankflag = stol(value);
				break;
		case EVBACKUP:	backup_flag = stol(value);
				break;
...
int PASCAL NEAR asc_int(char *st )
...
int PASCAL NEAR gettyp(	/* find the type of a passed token */
...
char *PASCAL NEAR getval( /* find the value of a token */
...
int PASCAL NEAR stol(	/* convert a string to a numeric logical */
...
char *PASCAL NEAR ltos(	/* numeric logical to string logical */
...
char *PASCAL NEAR mkupper(	/* make a string upper case */
...
char *PASCAL NEAR mklower(	/* make a string lower case */
...
int PASCAL NEAR absv( /* take the absolute value of an integer */
...
int PASCAL NEAR sindex( /* find pattern within source */
...
int PASCAL NEAR is_num(char *st )
...
#if	DEBUGM
int PASCAL NEAR dispvar(		/* display a variable's value */
...
pad(		/* pad a string to indicated length */
