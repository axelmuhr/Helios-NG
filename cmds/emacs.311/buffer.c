...
PASCAL NEAR nextbuffer(	/* switch to the next buffer in the buffer list */
...
PASCAL NEAR swbuffer(	/* make buffer BP current */
...
PASCAL NEAR popbuffer(

  int f,
  int n )	/* default and numeric arguments */
...
PASCAL NEAR zotbuf(	/* kill the buffer pointed to by bp */
...
PASCAL NEAR namebuffer(	/*	Rename the current buffer	*/

  int f,
  int n )		/* default Flag & Numeric arg */
...

{
	...
	int 		cfold;  /* current fold entry */ /* MJB: 11-Sep-89 */
	...
	if (cflag != FALSE) {
		...
		bp->b_key[0] = 0;
#endif
		bp->b_nfolds = 0;			   /* MJB: 11-Sep-89 */
		for (cfold = 0; cfold < NFOLDS; cfold++) { /* MJB: 11-Sep-89 */
			bp->b_folds[cfold].f_topline = NULL;
			bp->b_folds[cfold].f_botline = NULL;
		}
		lp->l_fp = lp;
		lp->l_bp = lp;
	}
...
PASCAL NEAR unmark(	/* unmark the current buffers change flag */

  int f,
  int n )	/* unused command arguments */
