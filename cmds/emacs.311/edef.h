...
NOSHARE char mainbuf[5] = "main";	/* name of main buffer		*/
...
NOSHARE int DNEAR backup_flag = HELIOS; /* create emacs.bak.		*/
		/* Default for Helios, not for any other system		*/
...
NOSHARE int killfoldbound;	/* kill buffer boundaries on fold */
NOSHARE int killedcr;		/* kill buffer contains a [NL] */

NOSHARE FOLDLINELIST	*linelist;	/* Fold lines. MJB: 15-Sep-89 */
...
NOSHARE extern short kbdm[ NKBDM ];	/* Holds kayboard macro data	*/
NOSHARE extern char *execstr;		/* pointer to string to execute */
NOSHARE extern char golabel[NPAT];	/* current line to go to	*/
NOSHARE extern char paralead[NPAT];	/* paragraph leadin chars	*/
NOSHARE extern char fmtlead[NPAT];	/* format command leadin chars	*/
NOSHARE extern char mainbuf[5];		/* name of main buffer		*/
NOSHARE extern char lterm[NSTRING];	/* line terminators on file write */
NOSHARE extern unsigned char wordlist[256];/* characters considered "in words" */
...
CONST extern char *modename[10];	/* text names of modes		*/
CONST extern char modecode[11];		/* letters to represent modes	*/
NOSHARE extern int DNEAR numfunc;	/* number of bindable functions */
NOSHARE extern KEYTAB keytab[NBINDS];	/* key bind to functions table	*/
...
NOSHARE	extern FOLDMARKENT *foldmarktab; /* Fold marker table. BG: 11-Oct-89 */

NOSHARE extern int DNEAR searchtype;	/* current search style		*/
NOSHARE extern int DNEAR yankflag;	/* current yank style		*/
NOSHARE extern int DNEAR backup_flag;	/* create emacs.bak files	*/
...
NOSHARE extern CONST char *cname[16];	/* names of colors		*/
...
CONST extern char errorm[6];		/* error literal		*/
CONST extern char truem[5];		/* true literal 		*/
CONST extern char falsem[6];		/* false litereal		*/
NOSHARE extern int DNEAR cmdstatus;	/* last command status		*/
NOSHARE extern char palstr[49];		/* palette string		*/
NOSHARE extern char lastmesg[NSTRING];		/* last message posted		*/
...
NOSHARE extern int killfoldbound;	/* kill buffer is fold bounded */
NOSHARE extern int killedcr;		/* kill buffer contains a [NL] */

NOSHARE extern FOLDLINELIST *linelist;	/* Fold lines. MJB: 15-Sep-89 */

NOSHARE	extern char lowcase[HICHAR];	/* lower casing map		*/
NOSHARE	extern char upcase[HICHAR];	/* upper casing map		*/

NOSHARE extern char pat[NPAT];		/* Search pattern		*/
NOSHARE extern char tap[NPAT];		/* Reversed pattern array.	*/
NOSHARE extern char rpat[NPAT];		/* replacement pattern		*/
...
CONST extern char *dname[10];		/* directive name table 	*/

#if	DEBUGM
/*	vars needed for macro debugging output	*/
NOSHARE extern char outline[NSTRING];	/* global string to hold debug line text */
#endif
