 ...
 */
PASCAL NEAR backword( int f, int n)
 ...
 */
PASCAL NEAR forwword( int f, int n)
 ...
 */
PASCAL NEAR endword( int f, int n)
 ...
 */
PASCAL NEAR upperword( int f, int n)
 ...
 */
PASCAL NEAR lowerword( int f, int n)
 ...
 * buffer. Bound to "M-C".
 */
PASCAL NEAR capword( int f, int n)
...
PASCAL NEAR fwdchar( register int f, register int n)
 ...
 * MJB: 05-Sep-90.
 */
PASCAL NEAR delfword(int f, int n)
 ...
 */
PASCAL NEAR delbword( int f, int n)
...
PASCAL NEAR fillpara( int f, int n)	/* Fill the current paragraph according to the
			   current fill column */
...
/*                      "%%Out of memory" */
	...
	*(--pp) = 0;	/* truncate the last space */
	...
	/* just in case we are at a foldline put newline in first & backup */
	lnewline();
	backchar(FALSE, 1);
	status = linstr(para);
	if (status == TRUE)	/* reposition us to the same place */
		status = backchar(FALSE, (back > 1) ? back - 1 : back);
...
PASCAL NEAR reform(	/* reformat a paragraph as stored in a string */
...
PASCAL NEAR killpara(	/* delete n paragraphs starting with the current one */
...
PASCAL NEAR wordcount( int f, int n)
