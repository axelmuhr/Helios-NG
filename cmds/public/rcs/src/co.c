...
#ifndef lint
static char rcsid[]=
"$Header: /usr/perihelion/Helios/cmds/gnu/rcs/src/RCS/co.c,v 4.12 90/06/18 14:52:40 chris Exp $ Purdue CS";
#endif
/*****************************************************************************
...
/* $Log:	co.c,v $
 * Revision 4.12  90/06/18  14:52:40  chris
 * Forgot 0222 Unixism
 * 
 * Revision 4.11  90/02/01  11:38:42  chris
 * Fix last fix
 * 
 * Revision 4.10  90/02/01  11:10:44  chris
 * Fix argument to chmod and creat
 * 
 * Revision 4.9  90/01/02  11:21:52  chris
 * Helios port
 * 
...
#define WWW (S_IWUSR|S_IWGRP|S_IWOTH)
                        if (chmod(RCSfilename,RCSstat.st_mode & ~WWW)<0)
 ...
 */
{
	...
	if ((workstat.st_mode & (S_IRUSR|S_IRGRP|S_IROTH))&&!forceflag) {    /* File is writable */
            if (!quietflag) {
                VOID rcsfprintf(stderr,"writable %s exists; overwrite? [ny](n): ",workfilename); fflush(stderr);
