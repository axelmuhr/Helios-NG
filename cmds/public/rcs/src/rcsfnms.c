...
#ifndef lint
 static char
 rcsid[]= "$Id: rcsfnms.c,v 4.11 90/02/01 11:11:54 chris Exp $ Purdue CS";
#endif
/****************************************************************************
...
/* $Log:	rcsfnms.c,v $
 * Revision 4.11  90/02/01  11:11:54  chris
 * Fix argument to chmod and creat
 * 
 * Revision 4.10  90/01/02  11:21:05  chris
 * Helios port
 * 
 ...
 */
{
	...
	register char *tp;
		...
		tfnames[freefilename] = talloc(L_tmpnam);
	return TMPNAM(tfnames[freefilename++]);
#else
 ...
 */
{
	...
	if (rindex(*argv,KDELIM)!=0) {
		...
		((finptr=fopen(RCSfilename=sub2filename,"r"))!=NULL) );
			...
			if ((access(RCSfilename=sub1filename,F_OK)==0) ||
			    (access(RCSfilename=sub2filename,F_OK)==0)) {
                                error("Can't open existing %s",RCSfilename);
                                return 0;
                        }
                        if (mustread) {
				...
				RCSfilename= (access(prefdir,F_OK)==0)?sub1filename:sub2filename;
                                returncode= -1;
                        }
                }
        }
...
#ifdef HELIOS
#include <sys/stat.h>
int rcsrename(from, to)
char * from, *to;
{	struct stat tostat;

	if( stat(to,&tostat) == 0 )
		chmod(to,tostat.st_mode|S_IWUSR);
	...
	{	FILE *fromf;
		FILE *tof;
		fromf = fopen(from,"r");
		if( fromf == NULL ) return 1;
		tof   = fopen(to,"w");
		if( tof == NULL ) { fclose(fromf); return 1; }
		fastcopy(fromf,tof);
		fclose(fromf);
		fclose(tof);
		remove(from);
...
#ifdef HELIOS
static char mytmpnam[L_tmpnam];
static int myserial = 0;
char *rcstmpnam(char *a)
{
	int signature = ((int)time(NULL)<<8) | (myserial++&0xff);
	if( a == NULL ) a = mytmpnam;
	sprintf(a,"T%07x",signature&0xfffffff);
	return a;
}
#endif
