 ...
 */
#ifndef lint
static char rcsid[]= "$Id: rcsutil.c,v 4.9 90/02/01 11:12:47 chris Exp $ Purdue CS";
#endif
...
/* $Log:	rcsutil.c,v $
 * Revision 4.9  90/02/01  11:12:47  chris
 * Fix argument to chmod and creat
 * 
 * Revision 4.8  90/01/02  11:22:44  chris
 * Helios port
 * 
...
{
	int ch;
	while((ch = fgetc(inf))!= EOF) putc(ch,outf);
}
#else
fastcopy(inf,outf)
FILE * inf, * outf;
...
static void tryopen(fd,file,flags)
	char *file;
{
	if (file  &&  fdreopen(fd,file,flags,S_IWUSR|S_IRUSR) != fd) {
...
#define M1 SHELL": not found"
		VOID write(fileno(stderr), M1, sizeof(M1));
		_exit(2);
	}
	return pid;
}
