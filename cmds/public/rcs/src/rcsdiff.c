...
#ifndef lint
static char rcsid[]=
"$Header: /usr/perihelion/Helios/cmds/gnu/rcs/src/RCS/rcsdiff.c,v 4.8 90/01/02 11:21:25 chris Exp $ Purdue CS";
#endif
/*****************************************************************************
...
/* $Log:	rcsdiff.c,v $
 * Revision 4.8  90/01/02  11:21:25  chris
 * Helios port
 * 
...
int argc; char **argv;
{
	...
	if (argc<1) fterror("No input file\n%s",cmdusage);
		...
		diagnose("===================================================================");
                ...
                if (revnums<2 && !(access(workfilename,R_OK)==0)) {
...
{       nerror++;
        VOID rcsfprintf(stderr,"%s error: ",cmdid);
	VOID rcsfprintf(stderr,e, e1, e2);
        VOID rcsfprintf(stderr,"\n%s aborted\n",cmdid);
        VOID cleanup();
	exit(ERRCODE);
}
