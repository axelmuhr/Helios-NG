...
#ifndef lint
static char rcsid[]=
"$Header: /usr/perihelion/Helios/cmds/gnu/rcs/src/RCS/rlog.c,v 4.9 90/01/02 11:21:31 chris Exp $ Purdue CS";
#endif
/*****************************************************************************
...
/* $Log:	rlog.c,v $
 * Revision 4.9  90/01/02  11:21:31  chris
 * Helios port
 * 
...
char * argv[];
{
	...
	caller=getcaller();
	    ...
	    if ( onlyRCSflag ) {
		VOID rcsfprintf(stdout, "%s\n", RCSfilename);
		goto loopend;
	    }
            /*   print RCS filename , working filename and optional
                 administrative information                         */
            VOID rcsfprintf(stdout, "\nRCS file:        %s;   ",RCSfilename);
            /* could use getfullRCSname() here, but that is very slow */
            VOID rcsfprintf(stdout, "Working file:    %s\n", workfilename);
            VOID rcsfprintf(stdout, "head:            %s\n", Head==nil?"":Head->num);
            VOID rcsfprintf(stdout, "branch:          %s\n", Dbranch==nil?"":Dbranch->num);

            VOID rcsfputs("locks:         ", stdout);  /*  print locker list   */
            currlock = Locks;
            while( currlock ) {
                VOID rcsfprintf(stdout,"  %s: %s;", currlock->login,
                                currlock->delta->num);
                currlock = currlock->nextlock;
            }
            if ( StrictLocks )
                VOID rcsfputs(Locks==nil?"  ;  strict":"  strict",stdout);

            VOID rcsfputs("\naccess list:   ", stdout);      /*  print access list  */
            curaccess = AccessList;
            while(curaccess) {
                VOID rcsfputs("  ",stdout);
                VOID rcsfputs(curaccess->login, stdout);
            ...
            curassoc = Symbols;
            while( curassoc ) {
                VOID rcsfprintf(stdout, "  %s: %s;",curassoc->symbol,
                           curassoc->delta->num);
            ...
            gettree();
            VOID rcsfprintf(stdout, "total revisions: %d;    ", TotalDeltas);
            if ( Head == nil || !selectflag || !descflag) {
                VOID putc('\n',stdout);
                if (descflag) VOID rcsfputs("description:\n", stdout);
                getdesc(descflag);
                VOID rcsfputs("=============================================================================\n",stdout);
                goto loopend;
            }
            ...
            if ( selectop || ( selectflag && descflag) )
                VOID rcsfprintf(stdout, "selected revisions: %d", revno);
            VOID putc('\n', stdout);
            if (descflag) VOID rcsfputs("description:\n", stdout);
            getdesc(descflag);
...
/*      trunk indicated whether this node is in trunk           */
{
        ...
        VOID rcsfprintf(stdout,"----------------------------\n");
        VOID rcsfprintf(stdout, "revision %s        ",node->num);
        if ( node->lockedby )
           VOID rcsfprintf(stdout, "locked by: %s;       ", node->lockedby);
        VOID putc('\n', stdout);

        VOID rcsfputs("date: ",stdout);
        VOID PRINTDATE(stdout,node->date); VOID putc(' ',stdout);
        VOID PRINTTIME(stdout,node->date);
        VOID rcsfprintf(stdout, ";  author: %s;  ", node->author);
        VOID rcsfprintf(stdout, "state: %s;  ", node->state);

        if ( editscript )
           if(trunk)
              VOID rcsfprintf(stdout,"lines added/del: %d/%d",
        ...
        if ( newbranch ) {
           VOID rcsfputs("branches:  ", stdout);
           while( newbranch ) {
                getbranchno(newbranch->hsh->num, branchnum);
                VOID rcsfprintf(stdout, "%s;  ", branchnum);
                newbranch = newbranch->nextbranch;
           }
           VOID putc('\n', stdout);
        }

        VOID rcsfputs(node->log,stdout);
}
