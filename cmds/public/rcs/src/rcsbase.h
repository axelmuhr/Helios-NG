 ...
 */
#define RCSBASE "$Id: rcsbase.h,v 4.12 90/02/01 11:13:25 chris Exp $"
...
/* $Log:	rcsbase.h,v $
 * Revision 4.12  90/02/01  11:13:25  chris
 * Fix argument to chmod and creat
 * 
 * Revision 4.11  90/01/02  11:46:22  chris
 * Helios port
 * 
...
#include <unistd.h>
#define getuid() 0
#define RENAME rcsrename
#define SHELL "/helios/bin/shell"
#define TMPNAM rcstmpnam
#else
...
#ifdef HELIOS
#define putc(ch, p) (--((p)->_ocnt) >= 0 ? (*((p)->_ptr)++ = (ch)) : fflsbuf(ch,p))
#else
...
#define WORKMODE(RCSmode) (RCSmode&~(S_IWUSR|S_IWGRP|S_IWOTH))|\
                            ((lockflag||!StrictLocks)?(S_IRUSR|S_IWUSR):0)
