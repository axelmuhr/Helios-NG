...
#ifndef lint
static char rcsid[]=
"$Header: /usr/perihelion/Helios/cmds/gnu/rcs/src/RCS/rcs.c,v 4.16 90/06/18 14:53:36 chris Exp $ Purdue CS";
#endif
...
/* $Log:	rcs.c,v $
 * Revision 4.16  90/06/18  14:53:36  chris
 * Flush messages
 * 
 * Revision 4.15  90/02/01  11:26:46  chris
 * Correct arg to umask as well
 * 
 * 
 * Revision 4.14  90/02/01  11:11:20  chris
 * Fix argument to chmod and creat
 * 
 * Revision 4.13  90/01/02  11:22:09  chris
 * Helios port
 * 
...
char * argv[];
{
	...
	caller=getcaller();
        ...
        oldumask = umask(S_IWUSR|S_IWGRP|S_IWOTH); /* turn off write bits */
...
#define WWW (S_IWUSR|S_IWGRP|S_IWOTH)
                result=chmod(RCSfilename,RCSstat.st_mode & ~WWW);
            elsif (haveworkstat==0)  /* initialization, and work file exists */
                result=chmod(RCSfilename,workstat.st_mode & ~WWW);
...
{       struct hshentry * nextdelta;
        ...
        do {
                ...
                if ( nextdelta->selector != DELETE) {
                        rewriteflag = true;
                        VOID rcsfprintf(frewrite,DELNUMFORM,nextdelta->num,Klog);
                }
 ...
 */
{
	...
	VOID rcsfprintf(stderr, "Revision %s is already locked by %s.\n", Delta, who);
        VOID rcsfprintf(stderr, "Do you want to break the lock? [ny](n): "); fflush(stderr);
        response=c=getchar();
	...
	VOID rcsfprintf(mailmess, "Subject: Broken lock on %s\n\n",bindex(RCSfilename,'/'));
        VOID rcsfprintf(mailmess, "Your lock on revision %s of file %s\n",Delta, getfullRCSname());
        VOID rcsfprintf(mailmess,"has been broken by %s for the following reason:\n",caller);
        VOID rcsfputs("State the reason for breaking the lock:\n", stderr);
        VOID rcsfputs("(terminate with ^D or single '.')\n>> ", stderr);
	fflush(stderr);
        old1 = '\n';    old2 = ' ';
        for (; ;) {
            c = getchar();
            if ( c == EOF ) {
                ...
                old2 = old1;   old1 = c;
                if (c== '\n') VOID rcsfputs(">> ", stderr); fflush(stderr);
            }
        }
        ffclose(mailmess);
...
#else /* MESSAGES */
	VOID rcsfprintf(stderr, "Helios cannot yet send mail\n"); fflush(stderr);
#endif	
	return(true);
}
        ...
        if ( cuthead )
           if ( cuthead->next == delstrt )
            ...
            if ( cuttail == nil && !quietflag) {
                VOID rcsfprintf(stderr,"Do you really want to delete all revisions ?[ny](n): "); fflush(stderr);
		c = response = getchar();
		while( c != EOF && c != '\n') c = getchar();
