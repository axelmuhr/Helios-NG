...
/* Act on #line and #file directives that may be inserted by a pre-processor, */
/* but leave in any directives inserted by user. */
int do_ppdirective()
{
	if (strncmp(cptr, "#line", 5) == 0) {
	    int i = 0;
	    char * ns;
	    ...
	    /* set input file */
	    /* will only work for #line xxx "filename" variety #line */

	    /* find start of filename */
	    while (*cptr != '\0' && *cptr++ != '"') ; /* null stat */
	    ...
	    /* set new filename */
	    strncpy(input_file_name, ns, i);
	    input_file_name[i] = '\0';

	    return TRUE;
	}

	if (strncmp(cptr, "#file", 5) == 0) {
	    int i = 0;
	    char * ns;

	    /* set input file */

	    /* will only work for #file "filename" variety #file */

	    /* find start of filename */
	    while (*cptr != '\0' && *cptr++ != '"') ; /* null stat */
		    
	    if (*cptr == '\0')
		return TRUE; /* maybe no filename specified, so ignore */
	...
	return FALSE;	/* pass on all unrecognised #xxx directives */
}
...
get_line()
{
	...
	if (c == '\n') {
		...
		/* Act on preprocessor directive, then get another line */
		/* if we acted and stripped #line or #file. */
		if ( do_ppdirective() == FALSE)
			return;
		i = 0;
		c = getc(f);
		if (c ==  EOF)
...
get_tag()
{
	...
	tagmax += 16;
	tag_table = (char **)
			(tag_table ? (char *) REALLOC(tag_table, tagmax*sizeof(char *))
				   : (char *) MALLOC(tagmax*sizeof(char *)));
...
initialize_grammar()
{
    nitems = 4;
    maxitems = 300;
    pitem = (bucket **) (char *) MALLOC(maxitems*sizeof(bucket *));
    if (pitem == 0) no_space();
    pitem[0] = 0;
    pitem[1] = 0;
    pitem[2] = 0;
