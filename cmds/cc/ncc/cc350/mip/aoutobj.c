...
/* Memo: the test of extindex>1 for codeseg/dataseg is nasty.              */
/*  This and the assumption that 2 symbols are ignored causes many         */
/*  TARGET_IS_ARM tests (if codeseg/dataseg are not the first two symbols  */
/*  then this dubious test kills).                                         */
...
static int obj_checksym(Symstr *s)
{   ExtRef *x = symext_(s);
    if (x != 0)
...
#ifdef TARGET_IS_ARM
        return (int)(idx - 2);  /* @@@ the pesky extindex>1 test again.      */
#else
        return (int)idx;
#endif
    }
...
/* 	  fprintf(stderr,"Found: rpt, sort, len, val = %ld, %ld, %ld, %lx\n", 
	  ...
	  if (p->sort != LIT_ADCON)
	    fprintf(stderr,"Not ADCON in Data Reloc %ld\n", p->sort);	
...
/* The following is a config/TARGET flag -- it has nothing really to    */
/* do with a.out.  The procedural interface is just confusing.          */
int obj_need_B_subsubmain(void)
{
    return(0);
}
...
/*
 * The spec of this procedure was designed for armobj and armdbg.
 * We cheat somewhat and interpret the pointer as a struct nlist *,
 * with a valid char *n_name field (or NULL), and the size_t n should be 0.
 * The object is copied and added to obj_stablist, to be written out with
 * the real symbol table entries later.
 */
...
void obj_writedebug(VoidStar p, size_t n)
{
    StabList *stab;
    if (n != 0) {
        syserr(syserr_aout_debug);
    }
    stab = (StabList *)SynAlloc(sizeof(StabList));
    stab->next  = obj_stablist; obj_stablist = stab;
    stab->nlist = *(struct nlist *)p;
    ++obj_stabcount;
}
...
#endif
#ifdef TARGET_IS_ARM
    h.a_syms   = (obj_symcount+obj_stabcount-2) * sizeof(struct nlist);
#else
