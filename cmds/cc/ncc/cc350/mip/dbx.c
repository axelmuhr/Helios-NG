/*
 * C compiler file dbx.c, version 4e
                     ...
                     unsigned long value)
{
    ...
    nlist.n_value     = value;
    obj_writedebug((VoidStar)&nlist, 0);
}
...
static int32 sprintstruct(char *pp, TypeExpr *type)
{
    ...
    if (tagbindmems_(b) == NULL) { /* no definition yet */
    ...
    {   char class = 0;
        switch (sort = tagbindsort_(b)) {
            case s_struct: class = 's'; break;
            case s_union:  class = 'u'; break;
            case s_enum:   class = 'e'; break;
            default:       syserr(syserr_tagbindsort,
                                  (long)tagbindsort_(b));
        }
        *pp++ = class;
        if (class != 'e') pp += sprintf(pp, "%ld", sizeoftype(type));
    }
    {   StructPos p;
        TagMemList *l;
...
static void dbg_addvar(Symstr *name, TypeExpr *t, int32 sourcepos,
                       VarClass class, int32 addr)
{
    dbg_addvar_t(name, dbg_typerep(t), sourcepos, class, addr);
}
...
/* are known to be held in global store).  (Does this matter?)             */
{
    ...
    {   DbgList *p, **prev = &dbglist;
        ...
        for (; (p = *prev) != NULL ; prev = &cdr_(p)) {
            int oldclass = p->car.DEB_VAR.class;
            if (p->debsort==D_VAR &&
...
static Dbg_LocList *dbg_lookupline(b)
         Binder   *b;
{   Dbg_LocList *p;
...
case bitofstg_(s_auto):
    ...
    if (debugging(DEBUG_Q)) cc_msg(" %c %lx", charofclass[class], (long)addr);
    dbg_addvar_t(name, loc->typerep, loc->pos, class, addr);
}
...
{   int32 oldlevel = length((List *)old);
     ...
     */
    for (nvars = 0; new != old; new = new->bllcdr)
    {   SynBindList *bl;
        if (new == 0) syserr(syserr_dbx_scope);
...
if ((c)>=0 && !sameline((x),curline)) \
    ...
    case D_VAR:
            {   char *pp = buf;
                ...
                if (p->car.DEB_VAR.class != CLASS_AUTO) {
                    *pp++ = charofclass[p->car.DEB_VAR.class];
                }
                pp += sprintf(pp, "%s", p->car.DEB_VAR.type);
                ntype = (int)ntypeof[p->car.DEB_VAR.class];
                *pp = 0;
