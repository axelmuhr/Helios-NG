...
/*
 * RCS $Revision: 1.1 $ Codemist 26
 * Checkin $Date: 1993/11/23 16:41:52 $
 * Revising $Author: nickc $
 */

#ifndef NO_VERSION_STRINGS
extern char codebuf_version[];
char codebuf_version[] = "\ncodebuf.c $Revision: 1.1 $ 26\n";
#endif
...
case LIT_BBBB:  return (u)pb[0] | (u)pb[1]<<8 | (u)pb[2]<<16 | (u)pb[3]<<24;
case LIT_HH:    return (u)ph[0] | (u)ph[1]<<16;
case LIT_BBH:   return (u)pb[0] | (u)pb[1]<<8 | (u)ph[1]<<16;
case LIT_HBB:   return (u)ph[0] | (u)pb[2]<<16 | (u)pb[3]<<24;
#endif
#ifdef TARGET_IS_BIG_ENDIAN
case LIT_BBBB:  return (u)pb[0]<<24 | (u)pb[1]<<16 | (u)pb[2]<<8 | (u)pb[3];
case LIT_HH:    return (u)ph[0]<<16 | (u)ph[1];
case LIT_HBB:   return (u)ph[0]<<16 | (u)pb[2]<<8 | (u)pb[3];
case LIT_BBH:   return (u)pb[0]<<24 | (u)pb[1]<<16 | (u)ph[1];
#endif
case LIT_OPCODE:
case LIT_NUMBER:
case LIT_ADCON:
...
void labeldata(Symstr *s)
{
    ...
    if (asmstream) /* nasty space-saving hack */
        adddata1(global_list3(SU_Data, 0, s, LIT_LABEL));
}
...
int32 byte_reverse(unsigned32 val, unsigned32 sort)
{
  unsigned32 b0, b1, b2;
/*
 * Here, I assume that in a cross-compilation environment, the code
 * and data will be subject to byte-reversal within words. Therefore
 * byte sequences must be pre-reversed and swapped with halfwords.
 * This function is complementary to that in armobj.c.
 */
  if (sort == LIT_HH) {
    ...
    b2 = (val >> 16) & 255;
    if (sort == LIT_HBB) {
      return (val << 16) | (b2 << 8) | (val >> 24);
    } else if (sort == LIT_BBH) {
      return (b0 << 24) | (b1 << 16) | (val >> 16);
    } else {
      /* LIT_BBBB == LIT_STRING */
      return (b0 << 24) | (b1 << 16) | (b2 << 8) | (val >> 24);
    }
  }
}
#endif
...
void show_code(Symstr *name)
{
  ...
  if (name != NULL)
    {   if (codep > maxprocsize)
        ...
        if (codebuf_inroutine && strcmp(symname_(name), currentfunction) == 0)
            end_of_fn = YES;
        else
            end_of_fn = NO;
    }
...
LabelNumber *nextlabel(void)
{
    ...
    w->block = (BlockHead *)(int)DUFF_ADDR;  /* unset label - illegal ptr */
...
/* See comments later in this file re x->codexrlitoff with X_backaddrlit    */
            if ((k & (unsigned long)0xff000000)==X_backaddrlit && x->codexrlitoff==offset)
...
void dump_count_names(void)
{
    ...
    outcodeword((unsigned long) 0xfff12340 | (count_name_pointer - 1L), LIT_NUMBER);
    for (i = 0; i<count_name_pointer; i++)
                       ...
                       int32 flag)
{
  ...
  if ((litpoolp >> LITPOOLSEGBITS) >= litveccnt)
	...
	litpool[litveccnt++] = (LitPoolEntry (*)[LITPOOLSEGSIZE]) BindAlloc(sizeof(*litpool[0]));
      else
        {
	  mustlitby = INFINITY;
...
static int32 nextstringword(StringSegList **s, int32 *ip, int32 *np)
{
    ...
    {   char *p = x->strsegbase;
        int32 len = x->strseglen, j;
        for (; i < len; i++) {
#ifdef REVERSE_OBJECT_BYTE_ORDER
    /*
     * Bizarrely, we have to swap the bytesex of the word here, because
     * it goes in the code area, which will have its bytesex swapped again.
     */
            j = 3 - n;
#else
            j = n;
 ...
 */
    outcodeword((unsigned long) 0xff000000 | (length + 4), LIT_NUMBER);
#endif
...
void codebuf_reinit(void)
{
    ...
    asm_lablist = NULL;
    litlab = (LabelNumber *)(int)DUFF_ADDR;
    litalign = 4;
#ifdef TARGET_IS_RISC_OS
    if (codebase > 0 &&
    ...
    {   int32 n = var_aof_code_area;
        ...
        bindsym_(codesegment) = sv = sym_insert_id(name);
        obj_symref(sv, xr_code + xr_defloc, 0L);
#ifndef NO_OBJECT_OUTPUT
