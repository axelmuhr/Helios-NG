...
/*
 * RCS $Revision: 1.1 $ Codemist 97
 * Checkin $Date: 1993/11/23 16:41:52 $
 * Revising $Author: nickc $
 */

#ifndef NO_VERSION_STRINGS
extern char cg_version[];
char cg_version[] = "\ncg.c $Revision: 1.1 $ 97\n";
#endif
...
#define NOTALABEL   ((LabelNumber *)(int)DUFF_ADDR)
...
static BlockHead *function_head;
static bool loopseen;

static int32 anylabels;         /* somewhat vestigial */
...
static VRegnum cg_stind(VRegnum r, Expr *x, const int32 flag,
                    const int32 mcmode, const int32 mclength, bool address);
...
static VRegnum open_compilable(Expr *x,RegSort rsort);
static VRegnum cg_fnap(Expr *x,VRegnum resreg,bool valneeded);
static VRegnum cg_cond(Expr *b,Expr *e1,Expr *e2,bool valneeded,
                       VRegnum targetreg,LabelNumber *l3,bool structload);
...
#define NOT_OPEN_COMPILABLE GAP
...
/* not legal here.                                                       */
            (void)cg_cond(arg1_(x), arg2_(x), arg3_(x), 1, r, l3, 0);
...
default:
        {   VRegnum r2 = cg_expr1(x, 1);
            RegSort rsort = regtype_(r);
            emitreg(rsort==DBLREG ? J_MOVDR :
                    rsort==FLTREG ? J_MOVFR : J_MOVR, r, GAP, r2);
            bfreeregister(r2);
        }
        break;
    }
...
#ifdef ADDRESS_REG_STUFF
            r = cg_var(GAP, (Binder *)x, s_content, mcmode,
                       mclength, rsort==ADDRREG);
            r = ensure_regtype(r,rsort);
#else
            r = cg_var(GAP, (Binder *)x, s_content, mcmode, mclength);
#endif
...
case s_valof:
            ...
            /* here and at s_valof (and cg_addr &c) to make them work.     */
                ...
                emitbinder(rsort==DBLREG ? J_INITD :
                           rsort==FLTREG ? J_INITF : J_INIT,
...
/* only legal here if the conditional expression as a whole is voided.   */
                r = cg_cond(arg1_(x), arg2_(x), arg3_(x), valneeded,
                            reserveregister(rsort), l3, 0);
                start_new_basic_block(l3);
...
case s_assign:
/* @@@ LDS 06-Oct-89: Codemist: remove this comment after reading it.  */
/* This use of cautious_mcmode and the whole load_integer_structure()    */
/* edifice seems misguided - I have removed it!! Please check.           */
#ifdef NEVER
            if (cautious_mcmode == 3)
            {   if (mcmode==0)
                    return cg_storein(load_integer_structure(arg2_(x)),
                                      arg1_(x), s_assign);
...
case s_content:
            if (!valneeded && !isvolatile_expr(x))
            ...
            {   Expr *fname =
                     ...
                     mclength==2 ? sim.readcheck2 :
                                   sim.readcheck4;
                x = mk_expr2(s_fnap, typeofexpr(x), fname, mkExprList(0, x));
            }
#ifdef ADDRESS_REG_STUFF
            r = cg_stind(GAP, x, s_content, mcmode, mclength, rsort==ADDRREG);
            r = ensure_regtype(r, rsort);
#else
            r = cg_stind(GAP, x, s_content, mcmode, mclength);
#endif
            if (!valneeded) bfreeregister(r), r = GAP;
            return r;
...
case s_plus:
        ...
        {   Expr *a1 = arg1_(x), *a2 = arg2_(x);
            if (h0_(a1) == s_neg)
                return cg_expr(mk_expr2(s_minus, type_(x), a2, arg1_(a1)));
            else if (h0_(a2) == s_neg)
                return cg_expr(mk_expr2(s_minus, type_(x), a1, arg1_(a2)));
            else if (!isintregtype_(rsort))
	      ...
	      {
		return(cg_binary(J_ADDR, a1, a2, 1, rsort));
	      }	    
        }
...
case s_minus:
        ...
        {   Expr *a1 = arg1_(x), *a2 = arg2_(x);
            if (h0_(a2) == s_neg)
                return cg_expr(mk_expr2(s_plus, type_(x), a1, arg1_(a2)));
            else if (!isintregtype_(rsort))
...
/* seems to make register usage behave better (see cg_binary_or_fn)      */
	  ...
	  */
                        ...
                        mkExprList(mkExprList(0, arg1_(x)), arg2_(x))));
#else
        ...
        else
        {
		  ...
		  {  
                ...
                {   LabelNumber *l = nextlabel();
                    emit(J_CMPK+Q_GT, GAP, r, 0);
                    emitbranch(J_B+Q_GT, l);
...
/* can't the unsignedness property get in rsort? */
        ...
        else
        {
            ...
            {   /* see above code and comments for s_div too                */
                ...
                /* if TARGET_HAS_CONDITIONAL_EXECUTION, this gives moderately
                 * uninspired code : what I really want is
                 *   CMPK; NEG+GT; AND; NEG+GT
                 * but I imagine this would cause intermediate bits of the
                 * back end some pain.
                 * AM: not at all -- it was designed (as was the code for
                 * unsigned fix) so that a simple upgrade to the code
                 * in mip/flowgraf.c which inserts conditional execution
                 * can do this.  Change use_cond_field() so that it behaves
                 * a little like 'diff' and instead of flattening (e.g.)
                 *   IFEQ(SEQ(a,b,c),SEQ(a',b,c'))
                 *   to  EQ+a, EQ+b, EQ+c, NE+a', NE+b, NE+c'
                 *   it generates instead
                 *       EQ+a, NE+a', b, EQ+c, NE+c'.
                 * (Basic blocks really are fundamental).
                 */
                emit(J_CMPK+Q_GT, GAP, r, 0);
                emitbranch(J_B+Q_GT, l);
...
{   RegList *p1;
    ...
    for (p1=fpregstosave; p1!=NULL; p1 = p1->rlcdr)
    {   VRegnum r = p1->rlcar;
        emitbinder((regtype_(r)==FLTREG ? j_f : j_d),
                   r, p2->bindlistcar);
        p2 = p2->bindlistcdr;
    }
}
...
typedef struct ArgInfo { Expr *expr; int info; int32 rep, addr; } ArgInfo;

static void cg_fnargs_stack(ArgInfo arg[], int32 intregargs, int32 n)
...
{   int32 narg;
    for (narg = n; narg!=intregargs; )
    {   int32 rep = arg[--narg].rep;
#if (alignof_double > alignof_int)
        if (arg[narg].addr + padsize(rep & MCR_SIZE_MASK, 4) !=
              arg[narg+1].addr)
...
/* cg_expr1(arg[narg].expr, 1); should work, but compiles wrong code (why?) */
          ...
          {   Expr *a = arg[narg].expr;
              ...
              (void)cg_exprreg(a, argr);
              usedregs = (RegList *) mkRegList(usedregs, argr);
              nusedregs++;
          }
#endif
          break;
...
/* The following code needs to know struct alignment as well as size:    */
            if (alignof_struct >= 4 && structsize < 20 &&
                !structure_function_value(arg[narg].expr))
/* The case of a (small) structure argument that does not involve a      */
/* function call can be compiled using a sequence of PUSH operations.    */
...
/* the last member.                                                       */
                ...
                {   VRegnum argr;
                    ...
                    argr = virtreg(R_A1 + NARGREGS1-1 - nusedregs, INTREG);
                    structsize -= 4;
                    if (b) emitvk(J_LDRVK, argr, structsize, b);
                    else emit(J_LDRK, argr, r, structsize);
                    usedregs = (RegList *) mkRegList(usedregs, argr);
                    nusedregs++;
                }
...
/* returning a structure is dealt with here...                           */
            { flush_arg_usedregs(arg[narg+1].addr);
...
/* usual position for the actual parameter.                             */
                ...
                BindList *nb = active_binders;
                current_stackdepth += padsize(structsize,4);
                for (i=padsize(structsize,4); i != 0; i-=4)
                    nb = mkBindList(nb, integer_binder);
...
/* state that I have just put it in.                                     */
    ...
    flush_arg_usedregs(arg[intregargs].addr);
#if (alignof_double > alignof_int)
/* The next few lines deal with the trickiness of handling a call like  */
/* f(1,2.3) which puts a double on the stack, then drops the stack,     */
/* loads 1 to an integer reg, then reloads 3 ints from stack.           */
/* This is not optimal code, but referencing a dead register (like A2)  */
/* in the above implicitly by the CALL(4words) could be nasty.          */
/* Because of the way the sentinel (q.v.) is arranged in cg_fnargs()    */
/* the test inhibits the fixup for an odd number of reg int args.       */
    if ((intregargs & 1) &&
          arg[intregargs-1].addr + 4 != arg[intregargs].addr)
        {   BindList *nb = active_binders;
            /* @@@ note that the next line of code assumes that         */
            /* (alignof_double/alignof_int) == 1 or 2.                  */
            nb = mkBindList(nb, integer_binder);
...
static void cg_fnargs_regs(ArgInfo arg[], int32 intregargs, bool normal)
...
/* f() { g(a(),b(),c(),...); }                                         */
    {   BindList *save_binders = active_binders;
        int32 d = current_stackdepth;
        VRegnum argregs[NARGREGS1];
        SynBindList *argsaves[NARGREGS1];  /* for previous fn call results */
        int hardness;
        int32 firsthard = -1;
        {   int32 i;
            for (i = intregargs; i != 0; )
            {   if (arg[--i].info == ISHARD) firsthard = i;
                argsaves[i] = (SynBindList *)(int) DUFF_ADDR, argregs[i] = GAP;
            }
        }
        for (hardness = ISHARD; hardness >= ISCONST; hardness--)
        {   int32 narg;
#ifdef TARGET_STRUCT_RESULT_REGISTER
# define callreg(n) (normal ? virtreg(R_A1+(n),INTREG) : \
                     (n)==0 ? virtreg(TARGET_STRUCT_RESULT_REGISTER,INTREG) : \
                     virtreg(R_A1+(n)-1,INTREG))
#else
# define callreg(n) virtreg(R_A1+(n),INTREG)
#endif
            for (narg = intregargs; narg!=0; )
                if (arg[--narg].info == hardness)
                {   Expr *a = arg[narg].expr;
                    if (hardness == ISCONST)
                        (void)cg_exprreg(a, callreg(narg));
                    else
                        argregs[narg] = cg_expr(a);
                    if (hardness == ISHARD)
                    {   SynBindList *b = narg == firsthard ? 0 : /* first easy */
                        ...
                        {   if (b->bindlistcdr != NULL)
                            ...
                            emitbinder(J_STRV, argregs[narg], b->bindlistcar);
        ...
        {   int32 i;
            ...
            {   SynBindList *b;
                ...
                else if (arg[i].info != ISCONST)  /* done above */
                {   VRegnum r = argregs[i];
...
static int32 cg_fnargs(ExprList *a, ArgInfo arg[], int32 n, bool normal)
/* normal is false if I have an implicit extra arg to indicate where to
 * put a structure result and that will NOT be passed as arg1
 */
{
    int32 argwords, intregargs = 0, fltregargs = 0;
    {   int32 argoff = 0, narg = 0;
#ifdef TARGET_STRUCT_RESULT_REGISTER
        int32 NARGREGS2 = NARGREGS;
        if (!normal) argoff = -4, NARGREGS2++;
/* to let honest arg1 have address 0 and allow for extra arg in register */
#else
#  define NARGREGS2 NARGREGS
#endif
        ...
        {   Expr *ae = exprcar_(a);
            ...
            argoff += padsize(rep & MCR_SIZE_MASK, 4);
            if (narg == intregargs && rep < 0x02000000 && narg < NARGREGS2)
                intregargs++;
        }
        if (n != narg || a != 0) syserr(syserr_cg_argcount);
#if (alignof_double > alignof_int)
...
/* after the last of an odd number of args.                             */
        ...
        argwords = argoff/4;
#ifdef TARGET_IS_MIPS                   /* generalise soon...           */
        while (fltregargs < 2 && fltregargs < narg &&
               (arg[fltregargs].rep >> MCR_SORT_SHIFT) == 2)
            fltregargs++;
#endif
    }
#if (alignof_double > alignof_int)
 ...
 */
    {   BindList *save_binders = active_binders;
        int32 d = current_stackdepth;
        int32 padcount = NARGREGS - argwords;
    ...
    cg_fnargs_stack(arg, intregargs, n);
    cg_fnargs_regs(arg, intregargs, normal);
...
/* the stack that I want to have in registers. Ditto structure values.   */
    ...
    {   RegList *q = NULL;
        int32 i;
        BindList *nb = active_binders;
        /* @@@ nov 88: kill the next three lines after a decent time: */
        for (i=R_A1; i<R_A1+argwords && i<R_A1+NARGREGS; i++)
            if (member(virtreg(i, INTREG), usedregs) != (i < R_A1+intregargs))
                syserr(syserr_cg_fnarg2);
        for (i=R_A1+intregargs; i<R_A1+argwords && i<R_A1+NARGREGS; i++)
            {   q = (RegList *) mkRegList(q, virtreg(i, INTREG));
	  ...
	  emitpush(J_POP, GAP, GAP, q);
...
#ifdef TARGET_IS_MIPS
        /* This produces horrid, but working, code.                     */
        for (i = fltregargs; i > 0; )
        {    i--;
             emitreg(J_MOVIDR, virtreg(R_FA1+i,DBLREG),
                               virtreg(R_A1+i*2,INTREG),
                               virtreg(R_A1+i*2+1,INTREG));
        }
#endif
        ...
        for (i=R_A1; i<R_A1+argwords && i<R_A1+NARGREGS; i++)
        {   usedregs = (RegList *) mkRegList(usedregs, virtreg(i, INTREG));
            nusedregs++;
        }
#ifdef TARGET_STRUCT_RESULT_REGISTER
        if (!normal) usedregs =
            (RegList *)mkRegList(usedregs,
...
/* 'normal' can only be FALSE if TARGET_STRUCT_RESULT_REGISTER is set.   */
    return argwords | fltregargs<<K_FPARG_SHIFT | (normal ? 0 : K_STRUCTRET);
}
...
#ifdef TARGET_STRUCT_RESULT_REGISTER
        if (h0_(x) != s_fnap) argwords = cg_fnargs(a, p, n, NO);
        else
#endif
        argwords = cg_fnargs(a, p, n, YES);
    }

/* Now the arguments are all in the right places - call the function     */

    {   Expr *fn = arg1_(x), *temp;
...
/* suppresses tail-recursion optimisation.                              */
        {   TypeExpr *t = princtype(typeofexpr(fn));
            if (h0_(t) == t_content && (t = princtype(typearg_(t)),
                h0_(t) == t_fnap && maxargs_(t) == 1999))
                    /* calling a va_arg function */
                    argwords |= K_VACALL;
        }
#endif
        if (h0_(fn)==s_addrof &&
            (temp = arg1_(fn))!=0 &&
            h0_(temp)==s_binder)
/* @@@ should argwords be "args in int regs" instead here?              */
    emitcall(J_CALLK, resreg, argwords | 
               (typefnaux_(bindtype_((Binder *)temp)).sideeffects > 0 ?
                  K_PURE : 0),
             (Binder *)temp);
        ...
        case 0x01000004:
            {   VRegnum r = cg_expr(fn);
                emitcallreg(J_CALLR, resreg, argwords, r);
                bfreeregister(r);
                break;
            }
        default:
...
#else
        return cg_var(GAP, (Binder *)x1, s_content, mcmode, arglength);
#endif
    }
    r = cg_expr(x1);
 ...
 */
	 ...
	 * XXX - NC - 29/07/91
	 *
	 * changed from 8 to NUM_BITS_PER_BYTE in statements below
	 */
	
        emit(J_SHLK+J_UNSIGNED, r1=fgetregister(INTREG), r,
                                NUM_BITS_PER_BYTE * (arglength-mclength));
        emit(J_SHRK+J_SIGNED, r1, r1, NUM_BITS_PER_BYTE * (arglength-mclength));
#endif
        bfreeregister(r);
        return r1;
case 1:     /* change width of (unsigned) */
	 ...
	 * XXX - NC - 29/07/91
	 *
	 * changed the following statement from
	 *
	 *	lowerbits( 8 * mclength )
	 *
	 * to
	 *
	 *	lowerbits( NUM_BITS_PER_BYTE * mclength )
	 */
	
        emit(J_ANDK, r1=fgetregister(INTREG), r, lowerbits( NUM_BITS_PER_BYTE * mclength));
        bfreeregister(r);
        return r1;
...
/*      if (arglength!=4) syserr(syserr_cg_cast3, (long)arglength); */
        ...
        {   VRegnum r2;
            emit(J_EORK, r2 = fgetregister(INTREG), r, (unsigned long)0x80000000);
...
/* THIS MUST BE DONE BY THE BACK-END SOON                                */
            {   VRegnum r2;
                ...
                emit(J_EORK, r1, r1, (unsigned long)0x80000000);
                start_new_basic_block(l2);
                bfreeregister(r2);
            }
        }
...
/* squash the result down to 16 bits.                                    */
        ...
        {   if (mcmode == 0)
            {
	      /*
	       * XXX - NC - 30/07/91 - POTENTIAL BUG
	       *
	       * changed 
	       *
	       *     8 * (4 - mclength)
	       *
	       * to 		(should the 4 be sizeof_int ?)
	       *
	       *     NUM_BITS_PER_BYTE * (4 - mclength)
	       */
	    
	      emit(J_SHLK+J_UNSIGNED, r1, r1, NUM_BITS_PER_BYTE * (4 - mclength));
	      emit(J_SHRK+J_SIGNED,   r1, r1, NUM_BITS_PER_BYTE * (4 - mclength));
            }
            else
                emit(J_ANDK, r1, r1, lowerbits( NUM_BITS_PER_BYTE * mclength ));
        }
        bfreeregister(r);
        return r1;
    }
    ...
    {
      /*
       * XXX - NC - 30/07/91 - POTENTAIL BUG
       *
       * changed
       *
       *     8 * (4 - mclength)
       *
       * to 		(should the 4's be sizeof_int's ?)
       *
       *     NUM_BITS_PER_BYTE * (sizeof_int - mclength)
       */
	    
        if (mclength == 4) return r;
        emit(J_SHLK+J_UNSIGNED, r1=fgetregister(INTREG), r, NUM_BITS_PER_BYTE * (4 - mclength));
        emit(J_SHRK+J_SIGNED, r1, r1, NUM_BITS_PER_BYTE * (4 - mclength));
        bfreeregister(r);
        return r1;
    }
...
/* The above values are the ONLY valid values for flag                   */
{
    ...
    RegSort rsort = mcmode!=2 ? INTREG : (mclength==4 ? FLTREG : DBLREG);
    TypeExpr *tt = prunetype(typeofexpr(x));
    /* YUK - the next line breaches lots of abstractions - fix soon */
    bool volatilep = h0_(tt)==t_content && isvolatile_type(typearg_(tt));
#ifdef ADDRESS_REG_STUFF
...
/* is_same(x1,x2) is not worth testing for addresses                     */
    ...
    r99 = (flag==s_displace || flag==s_content) ? fgetregister(rsort) : GAP;
    switch (mcmode)
    {
case 0: /* signed */
case 1: /* unsigned */
case 4: /* plain... i.e. neither signed nor unsigned */
        switch (mclength)
        {
/* Having a defined relationship between J_LDRBK and J_LDRBR (e.g.) ...  */
/* ... would help no end in the following code                           */
    case 1:
            if (flag==s_displace || flag==s_content)
            {   int32 s_u = mcmode == 4 ? 0 :
                            mcmode == 0 ? J_SIGNED : J_UNSIGNED;
                if (addrmode == AD_RD)
                  emit(J_LDRBK + s_u, r99, r1, n);
                else if (addrmode == AD_VD)
                  emitvk(J_LDRBVK + s_u, r99, n, x1b);
                else
                  emit5(J_LDRBR + s_u + down, r99, r1, r2, shift);
            }
            if (flag!=s_content)
            {   if (addrmode == AD_RD) emit(J_STRBK, r, r1, n);
                else if (addrmode == AD_VD) emitvk(J_STRBVK, r, n, x1b);
                else emit5(J_STRBR + down, r, r1, r2, shift);
            }
            break;
    case 2:
            if (flag == s_content || flag == s_displace)
            {   int32 s_u = mcmode == 4 ? 0 :
                            mcmode == 0 ? J_SIGNED : J_UNSIGNED;
                if (addrmode == AD_RD)
                  emit(J_LDRWK + s_u, r99, r1, n);
                else if (addrmode == AD_VD)
                  emitvk(J_LDRWVK + s_u, r99, n, x1b);
                else
                  emit5(J_LDRWR + s_u + down, r99, r1, r2, shift);
            }
            if (flag != s_content)
#ifndef TARGET_LACKS_HALFWORD_STORE
            {   if (addrmode == AD_RD) emit(J_STRWK, r, r1, n);
                else if (addrmode == AD_VD) emitvk(J_STRWVK, r, n, x1b);
                else emit5(J_STRWR + down, r, r1, r2, shift);
            }
#else
...
/* could be passed.                                                        */
		 ...
		 */
                ...
                emit(J_SHRK+J_SIGNED, rx, r, NUM_BITS_PER_BYTE);
    ...
    case 4:
            if (flag==s_content || flag==s_displace)
            {   if (addrmode == AD_RD) emit(J_LDRK, r99, r1, n);
                else if (addrmode == AD_VD) emitvk(J_LDRVK, r99, n, x1b);
                else emit5(J_LDRR + down, r99, r1, r2, shift);
            }
            if (flag != s_content)
            {   if (addrmode == AD_RD) emit(J_STRK, r, r1, n);
                else if (addrmode == AD_VD) emitvk(J_STRVK, r, n, x1b);
                else emit5(J_STRR + down, r, r1, r2, shift);
            }
            break;
    default:
            syserr(syserr_cg_bad_width, (long)mclength);
        }
        break;
case 2:
        if (flag==s_content || flag==s_displace)
        {   if (addrmode == AD_RD)
              emit(rsort==FLTREG ? J_LDRFK:J_LDRDK, r99, r1, n);
            else if (addrmode == AD_VD)
              emitvk(rsort==FLTREG ? J_LDRFVK:J_LDRDVK, r99, n, x1b);
            else
              emit5((rsort==FLTREG ? J_LDRFR:J_LDRDR)+down, r99, r1, r2, shift);
        }
        if (flag != s_content)
        {   if (addrmode == AD_RD)
              emit(rsort==FLTREG ? J_STRFK:J_STRDK, r, r1, n);
            else if (addrmode == AD_VD)
              emitvk(rsort==FLTREG ? J_STRFVK:J_STRDVK, r, n, x1b);
            else
              emit5((rsort==FLTREG ? J_STRFR:J_STRDR)+down, r, r1, r2, shift);
        }
        break;
default:
        syserr(syserr_cg_bad_mode, (long)mcmode);
        break;
    }
...
{   if (isintregtype_(regtype_(v)))
    {   usedregs = (RegList *) mkRegList(usedregs, v);
        if (++nusedregs <= NTEMPREGS+NARGREGS+NVARREGS) return v;
    }
    else
    {   usedfpregs = (RegList *) mkRegList(usedfpregs, v);
 ...
 */
                if (suppress_module == 1) return take_address((Expr *)b);
            }
#endif
#ifdef TARGET_LACKS_HALFWORD_STORE
...
/* Find a binder for the address involved...                             */
            ...
            {   bb_elt = bb->bindlistcar;
                if (bindaddr_(bb_elt) == base) break;
                bb = bb->bindlistcdr;
            }
            if (bb == NULL)
...
case bitofstg_(s_auto):
        ...
        {   /* We use LDRxV/STRxV for variables either whose address is  */
            ...
            {   r99 = fgetregister(rsort);
                emitbinder(rsort==DBLREG ? J_LDRDV :
                           rsort==FLTREG ? J_LDRFV : J_LDRV, r99, b);
            }
...
/* if only 8 or 16 bits is needed. Hence STRV does not need length data  */
            if (flag != s_content)
                emitbinder(rsort==DBLREG ? J_STRDV :
                           rsort==FLTREG ? J_STRFV : J_STRV, r, b);
            break;
        }
...
jolly:
        ...
        if (flag == s_content)    /* amalgamate */
          r99 = cg_stind(r, take_address((Expr *)b), flag,
                         mcmode, mclength, 0);
        else r = cg_stind(r, take_address((Expr *)b), flag,
                          mcmode, mclength, 0),
             flag = s_assign;
#else
        if (flag == s_content)    /* amalgamate */
            r99 = cg_stind(r, take_address((Expr *)b), flag, mcmode, mclength);
        else r = cg_stind(r, take_address((Expr *)b), flag, mcmode, mclength),
             flag = s_assign;
#endif
        break;
...
#ifdef ADDRESS_REG_STUFF
        if (flag == s_content)
             r99 = cg_stind(r, take_neat_address(b, rsort),
                            flag, mcmode, mclength, 0);
        else r = cg_stind(r, take_neat_address(b, rsort),
                          flag, mcmode, mclength, 0),
             flag = s_assign;
#else
        if (flag == s_content)
             r99 = cg_stind(r, take_neat_address(b, rsort),
                            flag, mcmode, mclength);
        else r = cg_stind(r, take_neat_address(b, rsort),
                          flag, mcmode, mclength),
             flag = s_assign;
#endif
...
{
    int32 mclength = mcrepofexpr(e);
    int32 mcmode = mclength >> MCR_SORT_SHIFT;
    bool volatilep = isvolatile_expr(e);
    VRegnum res = GAP;
    mclength &= MCR_SIZE_MASK;
...
#else
        res = cg_var(r, (Binder *)e, flag, mcmode, mclength);
#endif
        break;
...
case s_content:
        ...
        {   Expr *fname = mclength==1 ? sim.writecheck1 :
                         ...
                         sim.writecheck4;
            e = mk_expr2(s_fnap, typeofexpr(e), fname, mkExprList(0, e));
        }
#ifdef ADDRESS_REG_STUFF
        res = cg_stind(r, e, flag, mcmode, mclength, 0);
#else
        res = cg_stind(r, e, flag, mcmode, mclength);
#endif
        break;
default:
...
/* change from some earlier versions of this compiler - BEWARE           */
    a1 = mk_expr2(s_fnap, type, fname, mkExprList(mkExprList(0, a1), a2));
    return (cg_expr(a1));
}
...
/* Now I know that the initializer is 'var = i1'                         */
    ...
    x1 = arg1_(endtest);
    x2 = arg2_(endtest);
    if (x1==var && integer_constant(x2)) i2 = result2;
    else if (x2==var && integer_constant(x1)) i2 = i1, i1=result2;
    else return(0);
    unsignedp = unsigned_expression_(var);
...
static VRegnum open_compilable(Expr *x, RegSort rsort)
{
    ...
    if (h0_(fname) != s_addrof) return virtreg(0, NOT_OPEN_COMPILABLE);
    fname = arg1_(fname);
    if (fname == NULL || h0_(fname) != s_binder) return virtreg(0, NOT_OPEN_COMPILABLE);
/* Only consider this call if the function is a direct function name */
    ...
    if (narg == 1) {
        ...
        if (i) {
            ...
            emit(isintregtype_(rsort) ? J_INLINE1 :
                 rsort == FLTREG ? J_INLINE1F :
                                   J_INLINE1D,
                 targetreg, r1, i);
            bfreeregister(r1);
            return targetreg;
...
/* The idea here is that if we are doing both divide and remainder,
   ...
   This is a BIG problem for CSE.
   Using the normal CALLKP jopcode for this may be a bad idea.
 */
...
#ifdef TARGET_IS_ARM
#  ifndef TARGET_HAS_BLOCKMOVE
/* Small copies done inline with LOAD+STORE pairs; large ones outofline with
   a call to _memcpy.
 */
          if (n >= 20)
            return virtreg(0, NOT_OPEN_COMPILABLE);
#  else
...
#  endif
          ...
          { Binder *loadv = is_local_adcon(a2),
            ...
            r3 = fgetregister(INTREG);
            while ((n -= 4) >= 0)
            {   if (loadv) emitvk(J_LDRVK, r3, n, loadv);
                else emit(J_LDRK, r3, r2, n);
                if (storev) emitvk(J_STRVK, r3, n, storev);
                else emit(J_STRK, r3, r1, n);
            }
            bfreeregister(r3);
          }
        }
...
/* I synthesize a loop.                                                */
/* Again, for ARM it is difficult to produce a loop which is better    */
/* than a call on an intelligently coded _memset.  Again a lie.        */
        VRegnum r2;
#ifdef TARGET_IS_ARM
#  ifndef TARGET_HAS_BLOCKMOVE
        if (n >= 40)
...
#  endif
...
#ifdef TARGET_LACKS_RR_STORE
                emit(J_SUBK, r1, r1, 4);
                emitreg(J_STRK, r2, r1, 0);
#else
                emitreg(J_STRR, r2, r1, r3);
#endif
            emit(J_SUBK, r3, r3, 4);
        ...
        {   Binder *storev = is_local_adcon(a1);
            ...
            while ((n -= 4) >= 0)
            {
                if (storev) emitvk(J_STRVK, r2, n, storev);
                else emit(J_STRK, r2, r1, n);
            }
            bfreeregister(r2);
        }
#endif  /* TARGET_HAS_BLOCKMOVE */
...
static void init_slave_reg(Binder *b)
...
{
    int32 n = (bindaddr_(b) & ~BINDADDR_MASK) / 4;   /* known BINDADDR_ARG */
    VRegnum v = bindxx_(b);
    if (v != GAP)
    {
#ifdef TARGET_STRUCT_RESULT_REGISTER
...
#endif
        if (isintregtype_(regtype_(v)))
        {   if (n < NARGREGS) emitreg(J_MOVR, v, GAP, virtreg(R_P1+n,INTREG));
            else emitbinder(J_LDRV1, v, b);
        }
        else if (regtype_(v) == FLTREG)
/* J_MOVIFR has to copy a bit-pattern from an integer register into one  */
/* of the FP registers.                                                  */
        {   if (n < NARGREGS) emitreg(J_MOVIFR, v, GAP, virtreg(R_P1+n,INTREG));
            else emitbinder(J_LDRFV1, v, b);
        }
        else
...
/* register.                                                             */
        {   if (n < NARGREGS-1)
#ifdef TARGET_IS_MIPS
                if (n < 2*procinitfltargs)  /* already in an fp arg reg. */
                    emitreg(J_MOVDR, v, GAP, virtreg(R_FP1+n/2,DBLREG));
                else
#endif
                emitreg(J_MOVIDR, v, virtreg(R_P1+n,INTREG),
                                     virtreg(R_P1+n+1,INTREG));
            else
...
/* contiguous words. Then I can load the FP value easily.                */
            {   if (n == NARGREGS-1) procflags |= PROC_ARGPUSH;
...
static int32 cg_bindargs(BindList *x, bool ellipsis)
...
/* Things in an argument list can only have s_auto storage class     */
        ...
        if ((rep >> MCR_SORT_SHIFT) == 2 &&
                argoff == procinitfltargs*sizeof_double)
            procinitfltargs++;
#endif
        argoff += padsize(rep & MCR_SIZE_MASK, 4);
    }
 ...
 */
    ...
    lyingargwords = ellipsis ? 0x7fff : argoff/4;
    /* The operand of J_ENTER is used to determine addressing of args     */
    /* on machines such as the ARM.                                       */
    emit(J_ENTER, GAP, GAP, lyingargwords);
#ifdef TARGET_STRUCT_RESULT_REGISTER
    if (result_variable != NULL) init_slave_reg(result_variable);
#endif
    for (x1=x; x1!=NULL; x1=x1->bindlistcdr)
        init_slave_reg(x1->bindlistcar);
    active_binders = NULL;
...
/* return f(...), so use the reult variable directly to get whizzy code. */
                cg_exprvoid(mk_expr2(s_fnapstruct, te_void, arg1_(x),
                    mkExprList(exprfnargs_(x), (Expr *)result_variable)));
            else
...
/* of loop invariants.                                                   */
    ...
    if (!once) emitbranch(J_B, l2 = nextlabel());
...
/* the top of the loop.                                                  */
    cg_count(cg_current_cmd->fileline);
    if (pretest!=0) cg_test(pretest, 1, l1);
    else if (posttest!=0) cg_test(posttest, 1, l1);
    else emitbranch(J_B, l1);
...
/* this one.                                                             */
    ...
    {   blkflags_(blkhead) |= BLKLOOP;
        ...
        note_loop(this_loop, blkhead);
        if (oinloop)
        {   for (; this_loop!=NULL; this_loop = this_loop -> blklstcdr)
              othisloop =
                (BlockList *) mkBlockList(othisloop, this_loop -> blklstcar);
            this_loop = othisloop;
        }
        else in_loop = NO;
    }
...
/* The following code WAS guarded by:
>>>>    #ifdef TARGET_LACKS_3WAY_COMPARE
 * but recent improved optimisations in CSE caused condition-code setting
 * instructions to sometimes be moved between the two conditional
 * branches below (therefore corrupting condition codes).
 * @@@ review soon
 */
...
{   while (h0_(e)==s_comma)
    ...
    if (h0_(e)==s_cond) (void)cg_cond(arg1_(e), arg2_(e), arg3_(e),
                                    valneeded, targetreg, l3, structload);
    else if (structload)
    {   VRegnum r;
...
static VRegnum cg_cond(Expr *b, Expr *e1, Expr *e2, bool valneeded,
                       VRegnum targetreg, LabelNumber *l3,
                       bool structload)
{
    LabelNumber *l1 = nextlabel();
    cg_test(b, 0, l1);
...
case s_cond:
        {   LabelNumber *l3 = nextlabel();
            VRegnum r = cg_cond(arg1_(e), arg2_(e), arg3_(e), 1,
                                reserveregister(INTREG), l3, 1);
            start_new_basic_block(l3);
            return getreservedreg(r);
        }
...
/* result - however beware machines like the VAX where FP=INT reg!          */
         || nusedfpregs >= (NFLTARGREGS+NFLTTEMPREGS+NFLTVARREGS-sparefpregs-2)
#endif
         ) &&
        (op!=s_binder && op!=s_integer && op!=s_floatcon &&
         op!=s_string && op!=s_wstring))
    {
...
/* allocated register to make sure it does not clash with anything.      */
        if (r != GAP)
        {   VRegnum r1 = fgetregister(regtype_(r));
/* Register r is no longer formally allocated (by the chromaticity count */
/* code) but it will still exist!                                        */
            emitreg((regtype_(r)==DBLREG ? J_MOVDR :
                     regtype_(r)==FLTREG ? J_MOVFR : J_MOVR), r1, GAP, r);
            r = r1;
        }
...
static void topdec_init(void)
{
    ...
    procflags = 0;
    cg_infobodyflag = 0;
    cg_current_cmd = (Cmd *)(int)DUFF_ADDR;
    usedregs = usedfpregs = NULL;
    nusedregs = nusedfpregs = 0;
    spareregs = sparefpregs = 0;
    nreservedregs = 0;
...
/* The next two lines really ought to be done in syn.c and passed over. */
...
/* Object module generation may want to know if this module defines
   ...
   main() we should ensure that return; is mapped onto return 0; */
	    ...
	    /* IOdebug( "cg for %s", symname_( name ) ); */
...
/* @@@ LDS 06-Oct-89: Codemist: remove this comment after reading it.  */
/* Used to be cautious_mcrepoftype(), but this was misguided, I think.   */
                w = mcrepoftype(restype);
...
/* first argument that will point to where the result must be dumped.    */
                if ((w >> MCR_SORT_SHIFT) == 3)  /* structure result */
                {   TypeExpr *pt = ptrtotype_(restype);
                    result_variable = gentempbinder(pt);
	    ...
	    /* IOdebug( "cg: result type checked" ); */
...
/* scans it to set PROC_ARGPUSH.                                           */
	    ...
	    /* IOdebug( "cg: args bound" ); */
	    
            cg_cmd(x->v_f.fn.body);

	    /* IOdebug( "cg: cmds done" ); */
	    ...
	    /* IOdebug( "cg: infobody done" ); */
	    
            cg_return(0, !isprimtype_(restype, s_void));
	    
	    /* IOdebug( "cg: return done" ); */

/* Allow loop optimiser to collect adcons etc at top of function body.   */
#ifdef CG_FINDS_LOOPS
            note_loop(this_loop, function_head);
#endif
	    /* IOdebug( "cg: loops done" ); */
	    
            drop_local_store();
	    
	    /* IOdebug( "cg: store dropped" ); */
...
/* debugger *probably* does not need FP, but why bother??                */
	    ...
	    /* IOdebug( "cg: args bound (for debugger)" ); */
...
#endif
	    /* IOdebug( "cg: tables output" ); */
	    
            linearize_code();
	    
	    /* IOdebug( "cg: code linerized" ); */
	    
            dbg_xendproc(fl);
	    
            show_code(name);

	    /* IOdebug( "cg: code shown" ); */
...
void cg_init(void)
{
     ...
     * XXX - NC - 06/12/91
     *
     * The horrible hack below is to prevent the __codeseg label
     * from being produced in every file that goes to make up
     * a resident library
     */
