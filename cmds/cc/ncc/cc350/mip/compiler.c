...
static FILE *cc_open(const char *s, const char *mode)
{
	...
	freopen( file_name( stdout ), "wb", stdout )
#else
	stdout
#endif
	  : stdout;
...
case 'F': var_warn_implicit_fns = 0;
          break;
#ifdef TARGET_IS_C40
case 'X': new_suppress = D_VOIDRETURN;		break;
#endif
...
static void push_include(char *path, char *name, int flags)
...
static void set_compile_options(int argc, char *argv[])
{
  ...
  {   char *current = argv[count];
      ...
      if (current[0] == '-' && current[1] != '\0')	/* XXX - NC - 25/6/92 - second test added */
      {
...
#ifdef TARGET_HAS_PROFILE	/* XXX - added by NC 3/6/92 */
...
#ifdef TARGET_IS_C40
  if (suppress_module == 1 && ((var_cc_private_flags & 0x80) == 0))
    {
...
#ifdef C40_DEBUG
      IOdebug( "Ncc: compile: parsing" );
#endif
      
      d = rd_topdecl();

#ifdef C40_DEBUG
      IOdebug( "Ncc: compile: parsed" );
#endif
...
#ifdef C40_DEBUG
      IOdebug( "Ncc: compile: gen J_op codes" );
#endif
      
      cg_topdecl(d);

#ifdef C40_DEBUG
      IOdebug( "Ncc: compile: genned" );
#endif
...
#ifdef C40_DEBUG
      IOdebug( "Ncc: compile: func compiled" );
#endif
...
#ifdef TARGET_IS_HELIOS
#ifdef C40_DEBUG
  IOdebug( "Ncc: compile: making stubs" );
#endif
  
  obj_makestubs();            /* tentative positioning */

#ifdef C40_DEBUG
  IOdebug( "Ncc: compile: made" );
#endif
#endif
...
extern FILE *pp_inclopen(char *file, bool systemheader, bool *sf)
{
  ...
  {   PathElement *p;
      ...
      if (systemheader || (ccom_flags & FLG_USE_SYSTEM_PATH))
	{
	  p = p->link;
	}
...
/* to have the original 'file' and not the munged 'new_file' instore.   */
          ...
          {   strcpy(current, p->name);
              ...
              {   strcat(current, new_file);
                  ...
                  {   show_h_line(1, current, YES);
                      push_include(current, current, p->flags);
                      return new_include_file;
                  }
              }
          }
  ...
  else  /* rooted file */
  {
      ...
      {   show_h_line(1, new_file, YES);
          push_include(new_file, new_file, systemheader ? PE_SYS : PE_USER);
          return new_include_file;
      }
  }
  return 0;  /* failed */
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: banner printed" );
#endif
...
#define sig_ign ((void (*)(int))(SIG_IGN))
  ...
  suppress         = D_ENUM | D_SHORTWARN | D_STRUCTWARN | D_PPNOSYSINCLUDECHECK;
  sysdebugmask     = 0;
  syserr_behaviour = 0;
#ifdef TARGET_HAS_DEBUGGER
  usrdbgmask       = 0;
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: calling err_init" );
#endif
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: calling pp_init" );
#endif
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: pp_init returned" );
#endif
 ...
 */
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: processing args" );
#endif
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: displaying header" );
#endif

  show_h_line( 1, sourcefile, NO );

#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: initialising bindings" );
#endif

  bind_init();

#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: initialising lexical analyser" );
#endif

  lex_init();                        /* sets curlex.sym to s_nothing   */
  
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: initialising builtins" );
#endif

  builtin_init();                    /* change to setup from syn_init? */

#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: initialising semantic analyser" );
#endif
...
#ifndef PASCAL /*ECN*/
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: initialising syntax analyser" );
#endif
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: initialising code generator" );
#endif

  cg_init();

#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: most things initialised" );
#endif
  ...
  if (ccom_flags & FLG_COMPILE)
    ...
    {
#ifdef C40_DEBUG
      IOdebug( "Ncc: ccom: pre-processing" );
#endif
...
#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: compiled" );
#endif

  cleanup();

#ifdef C40_DEBUG
  IOdebug( "Ncc: ccom: cleaned up" );
#endif
