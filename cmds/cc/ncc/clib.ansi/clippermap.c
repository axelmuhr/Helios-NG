/* clippermap.c:  Copyright (C) Codemist Ltd         */
/* Very Clipper specific routines.		     */
...
/* displays the counts attached to a source listing of the original code.  */
    ...
    for (pass = 1; pass <=2; pass++)
    {
        ...
        for (p = (int)_codebase; p<(int)__mcount; p += 4)
        {   int w = *(int *)p;
            if ((w & 0xffff0000) == 0x453f0000 | (count&0xffff) &&
		(*(int *)(p+4) & 0xffff) == (((unsigned int)count)>>16))
				/* CALL SP, count */
            {   count_position k;
		int i;
		char *s;
