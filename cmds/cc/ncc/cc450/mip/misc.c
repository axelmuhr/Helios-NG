...
/*
 * RCS $Revision: 1.2 $  Codemist 63
 * Checkin $Date: 1995/05/19 13:58:14 $
...
{  AEop s;
   ...
   if (stg & bitoftype_(s_long))
   {   if (stg & bitoftype_(s_short))
          stg ^= bitoftype_(s_longlong) ^ bitoftype_(s_int) ^
                     (bitoftype_(s_long) | bitoftype_(s_short));
       stg &= ~bitoftype_(s_int);
   }
...
/* wrt format decoding could be discarded.                           ACN */
    ...
    case 'b':   /* ordinary binder, but works for tag binder too.       */
            {   Binder *b = va_arg(a, Binder *);
                errprintf("'%s'", b ? symname_(bindsym_(b)) : "<nullbinder>");
            }
            break;
