...
static void mode6orpc3(int32 arg, int32 mode)
{
   ...
   int32 iis, bds, is, bs;
    ...
    {  if ( d > 127 )
          d = byte_extend(d);
       asmprintf((pcmode ? "$%04x(PC,":"%d(A%d,"), d/*+(pcmode ? pcx : 0)*/, arg);
       w_index(a,reg,l,scl);
       asmprintf(")");
    }
    else {
      ...
      if (bds != 1) {
          if (bds == 3)
             asmprintf("$%x,", readlong() + (pcmode ? pcx : 0) );
          else
             asmprintf( (pcmode ? "%04x," : "%d,"),
...
static void ea(int32 op, instrword a, int32 w)
{
    ...
    switch (mode) {
        ...
        case 4: asmprintf("-(A%d)", arg);
                break;
        case 5: asmprintf("%d(A%d)", word_extend(readword()), arg);
                break;
        ...
        switch (arg) {
	        ...
	        {   int32 offset = readword();
		    if ((offset&0x8000)==0) asmprintf("$%04x.W", offset);
		    else asmprintf("$%08lx.W", offset|0xffff0000);
		}
                break;
...
case none:
	...
	if (op == dbcc) asmprintf("D1,"); /* Atari HiSoft requires this! */
	{
	  char buff[80];
    ...
    if ( bits(a, 7, 7))
       asmprintf("%d(A%d),D%d", readword(), bits(a, 2, 0), bits(a, 11, 9));
    else asmprintf("D%d,%d(A%d)", bits(a, 11, 9), readword(), bits(a, 2, 0));
    break;
