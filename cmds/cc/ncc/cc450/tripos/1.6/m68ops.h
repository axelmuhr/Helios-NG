...
/* C compiler file m68ops.h :  Copyright (C) A.Mycroft and A.C.Norman */
/* version 0.02b */
...
extern Stub *stublist;
extern int in_stubs;
extern void load_static_data_ptr(RealRegister r);
...
#define M_68KDATAREGS 0x07f
#define M_68KADDRREGS 0xf80
...
#define Mregnum_(x) ( (x)<R_ADDR1  ? (x) : \
                  ( (x)<=R_SP ? ((x)-6) :  \
                  ( ((x) == R_DS) ? 7 : \
                  ( ((x) == R_AS) ? 0 : -1 ))))

#define Mregtype_(x) ( ( ((x) <= (R_ADDR1-1)) | ((x) == R_DS)) ? RT_DATA : RT_ADDR )

#define datareg_(x)  (Mregtype(x) == RT_DATA)
#define addrreg_(x)  (Mregtype(x) == RT_ADDR)

/* Register values expressed in terms of the mask bit needed in MOVEM  */
#define LDregbit(x) (( 1 << (addrreg(x)? 8l: 0l)+Mregnum(x) ))

#define M_LDFP    LDregbit(R_FP)
#define M_LDAS    LDregbit(R_AS)
#define M_LDDS    LDregbit(R_DS)
                   
#define M_CPUREGS  (regbit(R_A1+NINTREGS)-regbit(R_A1))
#endif
