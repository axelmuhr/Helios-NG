...
{ /* N.B. the word 'pragma' is reserved by High-C (yuk).                */
  ...
  /*
   * Do this for transputers now that I'm adding memory access checking ???
   * Tony 3/1/95
   */
...
/*
 * Auxillary function to read an '@' file, string by string.
 * Tony 27/1/95.
 */
int at_read_string (FILE *	atfp,
		    char	string[])
{
    int		c;
    int		i;
    ...
    if (i == 256)
    {
	cc_msg ("string too long in at file\n");
    }
...
#define sig_ign ((void (*)(int))(SIG_IGN))
  ...
  /*
   * Dos @ file support (though it can also be used under decent operating
   * systems).  Tony 27/1/95.
   * @ file support - transfer all argv's into at_argv's, except that for any
   * string beginning with a "@" transfer all the stringas in the file into
   * at_argv.  Note that any "@" specified in an "@" file will not be themselves
   * replaced.  Tony 27/1/95.
   */
  {
      int	i;
      
      /* We allow for up to 2000 arguments */
      at_argv = (char **)(malloc (8000));	/* sizeof (char *) * 2000 */

      for (i = 0; i < 2000; i++) at_argv[i] = NULL;
      
      at_argc = 0;
     
      for (argv_pos = 0; argv_pos < argc; argv_pos++)
      {
	  char *	opt = argv[argv_pos];
	  ...
	  if (*opt == '@')
	  {
	      ...
	      if ((atfp = fopen (opt, "r")) == NULL)
	      {
		  continue;
	      }
	      while (at_read_string (atfp, atstring) != EOF)
	      {
		  at_argv[at_argc] = (char *)(malloc (strlen (atstring) + 1));
		  strcpy (at_argv[at_argc++], atstring);
	      }
	  }
	  else
	  {
	      at_argv[at_argc] = (char *)(malloc (strlen (argv[argv_pos]) + 1));
	      strcpy (at_argv[at_argc], argv[argv_pos]);
	      at_argc++;
	  }
      }
...
#if 0
      cc_msg ("Arguments are ... ");
      for (argv_pos = 0; argv_pos < argc; argv_pos++)
      {
	  cc_msg ("%s ", argv[argv_pos]);
      }
      cc_msg ("\n");
#endif
  }
