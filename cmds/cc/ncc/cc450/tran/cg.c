 ...
 */
/* $Id: cg.c,v 1.1 1995/05/19 11:38:17 nickc Exp $ */
...
#include "xpuops.h"
#include "cg.h"
#include "mcdep.h" /* XXX NC 19 May 1995    (for defn of DBG_ANY) */
#endif
/*}}}*/
/*{{{  Externs, defines etc */
...
static void cg_binary(AEop, Expr *, Expr *, int, RegSort);	/* Tony Mark */
...
/* scans it to set PROC_ARGPUSH.                                           */
            formals = binderise((BindList *)formals);
#ifdef __old
            if (dbgstream)
...
#endif
            cg_bindargs((BindList *)formals, x->v_f.fn.ellipsis);
            if (profile_option) emitprofile();
#if defined(DBX) || defined(TARGET_HAS_DEBUGGER)
#if 0
	    if( dump_info ) 
#else
            if (usrdbg( DBG_ANY ))      /* XXX NC 19 May 1995 */
#endif
                db_proc(b,formals,restype);
#endif
...
/* Force placing of disply onto the stack when performing memory access */
/* checking. Tony (22/12/94)						*/
	if( nextrefs > 1 || vstackused || memory_access_checks) 
		modtab = vlocal(8);
...
#else
	    /*
	     * If the operation is for unsigned integers (ie: mcmode==1)
	     * issue s_sum (defined in cg.h, not aeops.h), otherwise
	     * issue s_plus.  Tony 19/1/95.
	     */
	    cg_binary (mcmode==1?s_sum:s_plus, arg1_(x), arg2_(x), TRUE, INTREG);
#endif
            pop_trace(); return;
        }
...
default:
        cc_msg ("cg_expr1 (): op = %d\n", op);
        syserr("cg_expr1(%s)", symbol_name_(op));
        pop_trace(); return;
    }
}
/*}}}*/
...
AEop op;
{
  ...
  /* should never get here, but to keep the compiler happy ... */
  return -1;
}
/*}}}*/
/*}}}*/
