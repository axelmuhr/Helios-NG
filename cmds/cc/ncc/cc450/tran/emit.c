/*{{{  Includes */
/* $Id: emit.c,v 1.2 1995/08/04 11:19:48 nickc Exp $ */
#ifdef __old
#include "cchdr.h"
#include "AEops.h"
#else
...
/* 
 * Assumes that we are NOT compiling with lattice.  A separate
 * define causes the command line to be too long.
 * (I HATE DOS!!!!!!!)  Tony 25/1/95.
 */
#ifdef HOST_IS_DOS	
#define UNIONFIX 1
#endif
...
void emit (Xop, Operand);
#define emit_(x,op)	emit(x, (Operand)op)

void storeCode (Xop, Operand);
#define storeCode_(x,op)	storeCode (x, (Operand)op)

#else

void emit (Xop, long);
#define emit_(x,op)		emit (x, (long)op)
void storeCode (Xop, long);
#define storeCode_(x,op)	storeCode (x, (long)op)
...
StringSegList *s;
{
	...
	if( litpool == NULL ) syserr("Uninitialised literal pool");
	emit_( p_ldvl, litpool );
	emit_( f_ldnlp, offset );
}
/*}}}*/
void
emitFpConst (fc, rsort)
FPConst	fc;
RegSort	rsort;
{
  FloatCon * x = fc.d;
  emitFloatCon (x, rsort);
}
/*}}}*/
/*{{{  emitFloatCon */
void
...
RegSort rsort;
{
   if (floatingHardware )
   {
      ...
      else
      {
         ...
         if( litpool == NULL ) syserr("Uninitialised literal pool");
 	 emit_( p_ldvl, litpool ); pushInt();
 	 emit_( f_ldnlp, offset );
	 emit_( f_opr,rsort == FLTREG ? op_fpldnlsn : op_fpldnldb );
	 popInt();
      }
      pushFloat() ;
   }
   else
   {
	...
	if( litpool == NULL ) syserr("Uninitialised literal pool");
	emit_( p_ldvl, litpool ); pushInt();
	if( rsort == FLTREG ) emit_( f_ldnl, offset ); 
	else emit_( f_ldnlp, offset ); /* doubles are manipulated via pointers */
   }
}
/*}}}*/
/*{{{  addlitint */
...
RegSort rsort;
{
  ...
  bool found   = FALSE;
   ...
   else
   {
	...
	if (l->type == lit_floatSn)
	{
		l->v.fb.val = x->floatbin.fb.val;
	}
	else
	{	
		l->v.db.msd = x->floatbin.db.msd;
		l->v.db.lsd = x->floatbin.db.lsd;
	}
	l->offset = litpoolsize;
...
/* The real routine to store a code item into the code structure */
#ifndef UNIONFIX
void storeCode(op,opd)
Xop op;
Operand opd;
{
	Tcode *t;
#else
void storeCode(op,opdarg)
Xop op;
long opdarg;
{
	/* Lattice C gets union arguments wrong, the following is a fix */
