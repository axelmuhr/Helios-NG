/* $Id: cg.h,v 1.1 1995/05/19 11:38:17 nickc Exp $ */
/* Jim's defs for bool, true, false */

/* Tony fix 3/1/95 */
#ifdef CC450
#define CC420	1
#define LONGCOPY	1	/* Additional fix.  Tony 9/1/95 */
#endif
...
extern struct DEBUG {
	...
	/* Memory checking stubs Tony (22/12/94) */
	Binder *memcheck_loadword;
	Binder *memcheck_loadshort;
	Binder *memcheck_loadshortext;
	Binder *memcheck_loadbyte;
	Binder *memcheck_loadbyteext;
	Binder *memcheck_storeword;
	Binder *memcheck_storeshort;
	Binder *memcheck_storebyte;
...
#if 0
/*
 * Actually, we can't do it here as s_sum must satisy some macro's
 * defined on symbols in aeops.h, otherwise we have a fatal internal
 * error.  Currently, I've just added s_sum after s_plus in aeops.h.
 * Tony 23/1/95.
 */

/*
 * Add another symbol to represent unsigned addition.  s_NUMSYMS defined
 * in mip/aeops.h is the total number of symbold (defined there), so
 * define s_sum to be 10 more than that to ensure its uniqueness.
 * (Well that's the theory anyway.)  Tony 19/1/95.
 */
#define s_sum	s_NUMSYMS + 10
#endif
...
#else
#define lxxAllowed		(proc_type == 9 || memory_access_checks)
#endif
#define	gtuAllowed		(proc_type == 9)
#define macAllowed     		(memory_access_checks)
