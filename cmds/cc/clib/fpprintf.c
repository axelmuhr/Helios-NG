...
/* version 0.01a */
/* $Id: fpprintf.c,v 1.10 1992/10/26 16:36:14 nickc Exp $ */
...
/* being used. (However Helios/ARM uses these entry points exclusively)    */

#include "norcrosys.h"	/* for kernel_sdiv */
...
#if defined IEEE || defined C40FLT
  ...
  if (d == 0.0)
      ...
      if (w!=0)
	{
      ...
      if (hi < 100000)
        {
	  int 	loDiv100000;
	  int	midDiv100000 = _kernel_sdiv( 100000, mid );
...
/* Floating point version defined here rather than in time.c */

#define time_t int

/* difftime snuck in here from clib/time.c	*/
...
/* Integer versions of these fns have their names prefixed with an underbar */
...
#if defined(__ARM)
/* The following two fns are extracted directly from the fp math library */
/* math.c - keep in step! */

/* DO_NOT_SUPPORT_UNNORMALIZED_NUMBERS */
double frexp(d, lvn)
double d; int *lvn;
{
    fp_number d1;
