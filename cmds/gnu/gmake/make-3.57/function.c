...
static char *rcsid = "$Header: /usr/perihelion/Helios/cmds/gnu/gmake/make-3.57/RCS/function.c,v 1.1 90/08/28 14:35:11 james Exp $";
...
int
pattern_matches (pattern, percent, mword)
     register char *pattern, *percent, *mword;
{
  unsigned int len;
  ...
  if (percent == 0)
    {
      ...
      if (percent == 0)
	return streq (pattern, mword);
    }

  len = strlen (percent + 1);

  if (strlen (mword) < (percent - pattern) + len
      || strncmp (pattern, mword, percent - pattern))
    return 0;

  return !strcmp (percent + 1, mword + (strlen (mword) - len));
}

