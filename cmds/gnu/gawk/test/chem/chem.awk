...
function ringleave(last, d,	rd, verts) {	# return vertex of ring in dir d
	...
	rd = substr(last, 3)
	return sprintf("V%d.%s", int(reduce(d-rd)/(360/verts)+0.001) + 1, corner(d))
}

function corner(dir) {
	return dc[reduce(45 * int((dir+22.5)/45+0.0001))]
}	
...
function atom(s,    c, i, n, nsub, cloc, nsubc) { # convert CH3 to atom(...)
	...
	if (s ~ /([^0-9]\.)|(\.[^0-9])/)	# centered dot
		gsub(/\./, "\\v#-.3m#.\\v#.3m#", s)
	return sprintf("atom(\"%s\", %.6g, %.6g, %.6g, %.6g, %.6g, %.6g)",
