...
/* $Header: oldLineClipping.c,v 1.1 90/02/23 18:08:49 charles Locked $ */
/* $Source: /server/usr/users/charles/world/drawp/RCS/docs/oldLineClipping.c,v $ */
...
/*-------------------------------------------------------------*/
/*                            Priniting debugging information  */
/*-------------------------------------------------------------*/
...
/* The macro 'Coord' performs the initialization process by    */
/*   initializing 'Pc', the number of pixels to be plotted,    */
/*   and initializing 'Bn', the fractional error term for the  */
/*   first points, as well as reducing (x,y) to the            */
/*   co-ordinates of the first point.                          */
/* Note that after this routine has exited, (ex,ey) may not be */
/*   the exact end-point of the line, since the co-ordinate    */
/*   corresponding to the minor axis might be out by one pixel */
/*   in the direction of movement along the minor axis, this   */
/*   does not make the subsequent code work wrongly as the     */
/*   result is a 'worst case' result which is fine for the     */
/*   clipping optimization code which is the only code which   */
/*   makes use of the subsequent values.                       */
...
#    define InitCoords \
\
Dmaj = SgnMaj*(Emaj-Amaj);                                              \
Dmin = SgnMin*(Emin-Amin);                                              \
Amin += Half;                                                           \
Amaj --;                                                                \
Bn  =   (F(Amin)*I(Dmaj)+SgnMin*(Acc-1-F(Amaj))*I(Dmin));               \
Bn += I (F(Amin)*F(Dmaj)+SgnMin*(Acc-1-F(Amaj))*F(Dmin));               \
Amin=I(Amin); Amaj=I(Amaj)+1;                                           \
if((Bn=getBn(Bn))>=0) { Bn-=Dmaj; incMin(1) }                           \
if(capStyle==CapNotLast) Emaj--;                                        \
Emaj=I(Emaj); Pc=1+Emaj-Amaj; Emin=I(Emin+Half);
...
Bn  =   (F(Amin)*I(Dmaj)+SgnMin*(Acc-1-F(Amaj))*I(Dmin));               \
Bn += I (F(Amin)*F(Dmaj)+SgnMin*(Acc-1-F(Amaj))*F(Dmin));               \
Amin=I(Amin); Amaj=I(Amaj);                                             \
if((Bn=getBn(Bn))>=0) { Bn-=Dmaj; incMin(1) }                           \
if(capStyle!=CapNotLast) Emaj--;                                        \
Emaj=I(Emaj)+1; Pc=1+Amaj-Emaj; Emin=I(Emin+Half);
...
{  int Lx,Ly;   /* Limit values of current reigon              */
   ...
   plotFn = dpLookUpLineEntry[LmnX|LmnY|LmnM|lcb->mainGroup];
