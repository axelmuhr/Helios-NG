...
#ifndef cbreak		/* Sun curses.h has cbreak() */
...
static void initgame()
{
    ...
    for(i=0; i < BDEPTH; ++i)
	(void) mvprintw(PYBASE + i, PXBASE - 3,
		 "%c  .  .  .  .  .  .  .  .  .  .  %c",i+'A',i+'A');
    ...
    for(i=0; i < BDEPTH; ++i)
	(void) mvprintw(CYBASE + i, CXBASE - 3,
		 "%c  .  .  .  .  .  .  .  .  .  .  %c",i+'A',i+'A');
	...
	*cp = '\0';
	...
	{
	    prompt(1, "Placing the rest of your fleet at random...", NULL);
	    for (ss = plyship; ss < plyship + SHIPTYPES; ss++)
		if (!ss->placed)
		{
		    randomplace(PLAYER, ss);
    ...
    (void) prompt(0, "Press any key to start...", NULL);
    (void) getch();
}
...
int atcpu;
{
	...
	case '\014': (void)clearok(stdscr, TRUE); (void)refresh(); break;
	default:
	    if (atcpu)
...
static int plyturn()
{
    ...
    prompt(1, "Where do you want to shoot? ", NULL);
    for (;;)
    {
	(void) getcoord(COMPUTER);
	if (hits[PLAYER][curx][cury])
	{
	    prompt(1, "You shelled this spot already! Try again.", NULL);
	    beep();
	}
	else
	    break;
