...
extern vc40_probe(), vc40_attach();
extern vc40_open(), vc40_close(), vc40_read(), vc40_write();
extern vc40_ioctl(), vc40_reset(), vc40_select(), vc40_mmap();

extern struct vc40_units_struct *vc40_units[MAX_NVC40];
extern struct mb_device vc40cdevice[];
extern int logit;
extern int num_dsp;
...
#define	ERROR_MSG	logit=LOG_ERR ; log
#define	DEBUG_MSG	logit=LOG_DEBUG ; log

static void no_message() { }

#if DEBUG >= 9
#	define	DEBUG_MSG9	logit=LOG_DEBUG ; log
#else
#	define	DEBUG_MSG9	no_message
#endif
#if DEBUG >= 5
#	define	DEBUG_MSG5	logit=LOG_DEBUG ; log
#else
#	define	DEBUG_MSG5	no_message
#endif
#if DEBUG >= 3
#	define	DEBUG_MSG3	logit=LOG_DEBUG ; log
#else
#	define	DEBUG_MSG3	no_message
#endif
...
#define	CMD_HANDSHAKE(val)			\
	if (val) {				\
		CDELAY( ((val) == 0), 1000);	\
		if (val) {			\
			DEBUG_MSG(logit, "DSP1 command handshake failed\n"); \
			return (EBUSY);		\
		}				\
	}					\
