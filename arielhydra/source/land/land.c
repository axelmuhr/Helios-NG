...
int fdepth = DEPTH;
int watercolor=23, landcolor=80;
int minx = 5000;
int miny = 5000;
int maxx = 0;
int maxy = 0;
...
Frame	frame;
Canvas	canvas;
Panel_item	depth_item, engine_choice_item, contin_item, dsp_time, sparc_time;
...
/*		PANEL_NOTIFY_PROC, depth_proc, */
	...
	contin_item = (Panel_item) xv_create(panel, PANEL_CHECK_BOX,
		PANEL_LAYOUT, PANEL_VERTICAL,
		PANEL_CHOICE_STRINGS, "Continuous", NULL,
		PANEL_VALUE, contin_val,
		NULL);
...
Xv_singlecolor Palette_Array[];
{
    ...
    /*val = 38.0; */
    for (i=100;  i<111;  i++) {         /* reddish-brown */
        hsv2rgb(hue,sat,val,&ColorValue);
	   Palette_Array[i].red = ColorValue.Red;
	   Palette_Array[i].green = ColorValue.Green;
	   Palette_Array[i].blue = ColorValue.Blue;
    ...
    for(i=111, gval=0x20; i < 127; i++, gval++) {
       Palette_Array[i].red = 4*gval;  
       Palette_Array[i].green = 4*gval;
       Palette_Array[i].blue = 4*gval;
    }

    Palette_Array[127].red = 4*0x3f;  /* Set last DAC register to white */
    Palette_Array[127].green = 4*0x3f;
    Palette_Array[127].blue = 4*0x3f;
}
...
int get_contin()
{
	return ((int) xv_get(contin_item, PANEL_VALUE));
}
