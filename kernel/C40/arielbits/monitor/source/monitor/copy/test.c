...
int test( hydra_conf config )
{
	...
	parms[2] = length;
	copy( parms );
	if( !MemTest( 0xc0000000, config.sram1_size.global*1024, &MemTestResults ) )
	{
		parms[0] = dram;
		parms[1] = g_sram;
		parms[2] = length;
...
*/
	...
	for( i=0 ; i < k ; i++ )
	{
		...
		c40_printf( "Testing Local SRAM on DSP %d ... ", 2+i );
		switch( i )
		{
			case 0 :
				comm_sen( 0, config.sram2_size.local, NumTries );
				break;
			case 1 :
				comm_sen( 1, config.sram3_size.local, NumTries );
				break;
			case 2 :
				comm_sen( 2, config.sram4_size.local, NumTries );
				break;
		}
		if( !comm_rec( i, &fail_addr, NumTries ) )
		{
		...
		c40_printf( "Testing Global SRAM on DSP %d ... ", 2+i );
		switch( i )
		{
			case 0 :
				comm_sen( 0, config.sram2_size.global, NumTries );
				break;
			case 1 :
				comm_sen( 1, config.sram3_size.global, NumTries );
				break;
			case 2 :
				comm_sen( 2, config.sram4_size.global, NumTries );
				break;
		}
		if( !comm_rec( i, &fail_addr, NumTries ) )
		{
