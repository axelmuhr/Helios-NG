			...
			case SetProperty:
				...
				switch( i & 0xF0 )
				{
					...
					case CpuClock:
						...
						break;
					case LocalSRAM1Size:
						config.sram1_size.local = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case GlobalSRAM1Size:
						config.sram1_size.global = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case LocalSRAM2Size:
						config.sram2_size.local = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case GlobalSRAM2Size:
						config.sram2_size.global = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case LocalSRAM3Size:
						config.sram3_size.local = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case GlobalSRAM3Size:
						config.sram3_size.global = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case LocalSRAM4Size:
						config.sram4_size.local = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case GlobalSRAM4Size:
						config.sram4_size.global = ReadHostWord() & 0xFFFF;
						write_config( config );
						break;
					case DRAMBase:
			...
			case GetProperty:
				...
				switch( i )
				{
					...
					case CpuClock:
						...
						break;
					case LocalSRAM1Size:
						WriteHostWord( config.sram1_size.local );
						writeVIC( 0x63, SUCCESS );
						break;
					case GlobalSRAM1Size:
						WriteHostWord( config.sram1_size.global );
						writeVIC( 0x63, SUCCESS );
						break;
					case LocalSRAM2Size:
						WriteHostWord( config.sram2_size.local );
						writeVIC( 0x63, SUCCESS );
						break;
					case GlobalSRAM2Size:
						WriteHostWord( config.sram2_size.global );
						writeVIC( 0x63, SUCCESS );
						break;
					case LocalSRAM3Size:
						WriteHostWord( config.sram3_size.local );
						writeVIC( 0x63, SUCCESS );
						break;
					case GlobalSRAM3Size:
						WriteHostWord( config.sram3_size.global );
						writeVIC( 0x63, SUCCESS );
						break;
					case LocalSRAM4Size:
						WriteHostWord( config.sram4_size.local );
						writeVIC( 0x63, SUCCESS );
						break;
					case GlobalSRAM4Size:
						WriteHostWord( config.sram4_size.global );
						writeVIC( 0x63, SUCCESS );
						break;
					case DRAMBase:
	...
	else
	{
		switch( msg->WhatToDo )
		{
			...
			case HostIntVector:
				...
				break;
			case DisableKeyInt:
				writeVIC( 0x27, readVIC(0x27) | 0x80);
				msg->WhatToDo = SUCCESS;
				break;	
			case EnableKeyInt:
				writeVIC( 0x27, readVIC(0x27) & ~(0x80));
...
int write_config( hydra_conf config )
{
	...
	WriteEeprom( 71, config.sram1_size.local, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 72, config.sram1_size.global, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 73, config.sram2_size.local, config.l_jtag_base+8);
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 74, config.sram2_size.global, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 75, config.sram3_size.local, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 76, config.sram3_size.global, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 77, config.sram4_size.local, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	WriteEeprom( 78, config.sram4_size.global, config.l_jtag_base+8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
	...
	WriteEeprom( 0x8, 0x10 | (config.l_jtag_space<<2), config.l_jtag_base + 8 );
	if( !EEPROM_Status )
	{
		return( 0 );
	}
