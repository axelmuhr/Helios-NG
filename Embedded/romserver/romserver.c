...
/* $Id: romserver.c,v 1.1 1994/07/05 09:58:36 tony Exp tony $ */
 ...
 *     until T_RomDisk is found - In which case ROMBASEDNUCLEUS is required.
 */
...
#define FILEBASED
#undef FILEBASEDFILE
#define FILEBASEDFILE "/c/romdir.fil"

/* Set default to ROMBASED */
#ifndef FILEBASED
#  ifndef ROMBASED
#      define ROMBASED
#  endif
...
/*
 *		SPECIFIC STRUCTURE / DEFINITIONS (dependent on type of romdisk)
 */

#ifdef FILEBASED

/* default file holding romdisk - romdir.fil in current directory */
...
#define CloseRom(rominfo) \
		Close (rominfo -> instream); Free (rominfo)

#else	/* ROMBASED */

/* set default to ROMBASEDFILE - default file is romdir.fil in current directory */
# ifndef ROMBASEDFILE
#  ifndef ROMBASEDNUCLEUS
#    define ROMBASEDFILE "romdir.fil"
#  endif
#endif
...
{
#ifndef ROMBASEDNUCLEUS
	struct ImageHdr	ihdr;		/* image header is only in files */
#endif
...
static ROMINFO * OpenRom (void)
{
	...
	r -> instream.rombuffer = (Mptr)(Malloc (size));
	if (r -> instream.rombuffer == NULL (Mptr))
	{
		IOdebug ("rom: failed to allocate memory for ROM buffer");
	...
	/*
	 * Find the romdisk in the nucleus by looking at all entries in the vector
	 * table and examining their type for T_RomDisk
	 */
	...
	while (MWord_(vector_ptr, 0))
	{
		...
		IOdebug ("ModuleWord_(p, Type) = %x", ModuleWord_(p, Type));
#endif
	...
	r -> instream.rombuffer = p;

#ifdef ROM_DEBUG
	IOdebug ("rom: OpenRom () - rombuffer starts at %x",
		 		(long)(r -> instream.rombuffer));
#endif
...
static int ReadRom (ROMINFO * 	stream,
		    ...
		    WORD 	timeout)
{
	MPtr	mp = stream -> instream.rombuffer;
	word	offset = stream -> instream.pos;

	timeout = timeout;	/* keep the compiler happy */

#ifdef ROM_DEBUG
	IOdebug ("rom: ReadRom () - reading %d bytes from offset %d (0x%x)", size, offset, offset);
	IOdebug ("rom: ReadRom () - mp     = %x", (long)(mp));
	IOdebug ("rom: ReadRom () - offset = %d", offset);

	IOdebug ("rom: ReadRom () - current word is %x", MWord_(mp, 0));
#endif

	MData_(buffer, mp, offset, size);
...
static ObjNode * getobjnode()
{
	...
	if (ReadRom (RomInfo, (BYTE *)objnode, (WORD)(sizeof (ObjNode)), -1) != sizeof (ObjNode))
	{
...
#ifdef ROM_DEBUG
	IOdebug ("rom: objnode %s %d", objnode->Name, objnode->Type);
#endif
			   ...
			   WORD		number)
{
	...
	while (number-- > 0)
	{
		...
		{
#ifdef ROM_DEBUG
			IOdebug ("rom: extract_files () - found DirNode %s",
				    	objnode -> Name);
#endif

			/* If a directory, recurse down to all its entries */
...
#ifdef ROM_DEBUG
			IOdebug ("rom: extract_files () - found ObjNode %s",
					objnode -> Name);
#endif
...
#endif
			addr = MInc_(RomInfo -> instream.rombuffer, offset);

#ifdef ROM_DEBUG
			IOdebug ("rom: %s appears at %x in romdisk",
					objnode -> Name, (long)addr);
			IOdebug ("rom: MWord_(addr, 0) = %x", MWord_(addr, 0));
#endif
			 ...
			 ObjNode * 	f)
{
	...
	for (i = 0; i < size; i += 65535)
	{
		...
		{
			WORD   	j;
			MPtr	buf = MInc_(RomInfo -> instream.rombuffer, pos + offset + 1);
			static char	buffer[65536];

			/* Alenia bug fix. */
			for(j= 0; j < m -> MsgHdr.DataSize ; j++)
			{
				MData_(buffer, buf, j, 1);
			}
			m -> Data = buffer;
		}
		PutMsg (m);
...
static void do_delete (ServInfo *	servinfo)
{
	...
	if (obj -> Type == Type_Directory && ((DirNode *)obj) -> Nentries != 0)
	{
...
int main (void)
{
	...
	if ((res = ReadRom (RomInfo,
	    		    (BYTE *) &(RomInfo -> ihdr),
	                    (WORD)(sizeof (struct ImageHdr)),
			    -1)) != sizeof (struct ImageHdr))
	{
...
#endif /* ROMBASEDNUCLEUS */

	if ((res = ReadRom (RomInfo,
			    (BYTE *) &RomInfo -> modhdr,
			    (WORD)(sizeof (struct Module)),
	...
	else
	{
		...
		IOdebug ("rom: Module Type = %x", RomInfo -> modhdr.Type);
		IOdebug ("rom: Module Name = %s", RomInfo -> modhdr.Name);
	}
#endif
	...
	else
	{
		...
		IOdebug ("rom: Root.Name = %s", Root.Name);
		IOdebug ("rom: Root.Entries = %d", Root.Entries);
	}
#endif
...

	{
		...
		LinkNode *	parent;
		Object *	me = Locate (Null(Object), ProcessorName);

		parent = (LinkNode *)Malloc (sizeof (LinkNode) + strlen (ProcessorName));
		...
		strcpy (parent -> Link, ProcessorName);
