...
AnsiInit (char *term)
{
    ...
    if (*term)
    {
    	...
    	{
   	case -1 :
	    IOdebug ("%s : unable to open termcap database.", fname);
	    return FALSE;
    	case  0 :
#ifdef	DEBUG
...
AnsiInitScreen (Screen *s)
{
    ...
    if ((map = (char **) Malloc (term_rows * sizeof (char *))) == NULL)
	return FALSE;

    if ((tempmap = (char **) Malloc (term_rows * sizeof (char *))) == NULL)
    {
    	Free (map);
	return FALSE;
    }

    if ((screen = (char *) Malloc (term_rows * term_cols)) == NULL)
    {
	Free (map);
	Free (tempmap);
	return FALSE;
