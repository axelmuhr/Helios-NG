...
static word
fillBuffer ( Stream *s, char *inbuf, char *ip, word *cnt )
{
    char *	ptmp;
    word	stat;
...
getEntry ( Stream *s, char *outbuf, char *name )
{
    ...
    word	eof	= FALSE;	/* end of file reached ?	*/
    word	cnt	= 0;		/* # unprocessed chars in inbuf	*/
	...
	elif ( strncmp( name, ip, (int)namlen ) == 0 )	/* terminal name found	*/
	{
#ifdef	DEBUG
...
tgetent ( char *bp, char *name )
{
    ...
    termcap = getenv ("TERMCAP");	/* scan user's environment	*/
