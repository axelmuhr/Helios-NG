...
#include "tty.h"
extern word device_read(void *buf, int size, word timeout);
...
GetKey (word timeout)
{
    ...
    do
    {
	error = device_read (data, 1, timelimit);
#ifdef	DEBUG
...
HandleInput (void)
{
	...
	chw = GetKey (NoTimeout);	/* get next character	*/
...
InputInit (void)
{
	...
	if (temp != NULL)
	{
	    if ((node = (key_node *) Malloc (sizeof (key_node))) == NULL)
	    {
#ifdef	DEBUG
    ...
    if ((key_table = (key_def *) Malloc ((word)keys * sizeof (key_def))) == NULL)
    {
#ifdef	DEBUG
