...
** tty.h								**
**									**
**	- Definitions and prototyping for the Terminal Window Server	**
**									**
**************************************************************************
** HISTORY   :	most of the original file window.h (date 08/08/90)	**
** -----------								**
**	     :	12/09/90 : G. Jodlauk					**
*************************************************************************/
...
#define ShortTimeout	OneSec / 4	/* timeout for a CSI		*/
...
typedef struct Window
{
    ...
    word		SelectMode;	/* current selection mode	*/
    Port		SelectPort;	/* current select Port		*/
    Semaphore		ReadLock;	/* prevent multiple Read reqs.	*/
    Semaphore		WriteLock;	/* prevent multiple Write reqs.	*/
    Semaphore		SelectLock;	/* lock on select variables	*/
...
*/
    bool		deleted;	/* set by Do_Delete()		*/
}
Window;

/*-------------------------  Global constants  -------------------------*/

extern tcap_def tcap_table [ 60 ];		/* keys to look for in termcap	*/
extern char	*tcap_null;

/*-------------------------  Global variables  -------------------------*/

extern int		terminating;	/* terminating flag		*/
extern char		**environ;	/* Environment pointer		*/
extern char		term_name[ 32 ];/* name of terminal		*/
extern char		my_nte[ 32 ];	/* name table entry		*/
extern DCB		*termdcb;	/* DCB of terminal device	*/
...
extern Semaphore	Attr_Lock;	/* lock for Attributes		*/
...
extern char		tcap_data[ 1024 ]; /* termcap data buffer	*/
extern char		tcap_buff[ 1024 ]; /* termcap string buffer	*/
...
extern void		WindowSelect	( MCB *m, Window *w );
extern void		WindowGetInfo	( MCB *m, Window *w );
extern void		WindowSetInfo	( MCB *m, Window *w );
extern void		DeviceWindowSetInfo (Attributes *a);
extern void		DeviceWindowGetInfo (Attributes *a);
