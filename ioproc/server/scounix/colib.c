...
WORD            size;
{
	int i;
        ...
        /* ServerDebug("@CreateCo (fn, stacksize = %d (0x%lx)", size, size); */

        /* get memory for coroutine vars, context save area AND stack */

        if ( !( co_buffer = (co_routine *)malloc(sizeof(co_routine))  ))
	{
		ServerDebug ("CreateCo () - cobuffer is NULL");
                return NULL;
	}
	...
	if ( !( co_buffer->co_sp = cosalloc(size)  ))
	  {
		...
		ServerDebug ("CreateCo () - co_buffer->co_sp is NULL");
                return NULL;
	  }
