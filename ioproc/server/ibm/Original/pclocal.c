...
{ word temp, conf_base, dma_chan;
  ...
  if  (host eq NULL || box eq NULL)
    { ServerDebug("Missing entries in configuration file for HOST and/or BOX.");
...
#if Ether_supported
  { extern void Ether_InitServer(); //initialise_ethernet();
    Ether_InitServer(); //initialise_ethernet();
  }
#endif
