 ...
 *
 * bits 0,1,2	IRQ channel: the number must be in the range of the
 *		allowable interrupts (ie 2-7).
 *
 * bits 3-10	Base address: this number must be in the range
 *		0 - 126 (ie: 0x00 - 0x7E) and is multiplied by 32 (0x20)
 *		to get the actual base address.
 */

/*
 * Macro to extract unit number, base address and interrupt channel
 * from device information.
 */
#define BBK_UNITNUM(dev)	((minor(dev) >> 3) & 0xFF)
#define BBK_BASEADDR(dev)	(struct bbk_reg *)(BBK_UNITNUM(dev) * 0x20)
#define BBK_INTCHAN(dev)	(minor(dev) & 0x7)

#define UNIT_TO_REG(unit)	(struct bbk_reg *)(unit * 0x20)

/*
 * Using the above scheme there can be a maximum number 
 * of 126 devices.
 */
#define NBBK	126
...
/*
 * Macro to print printf messages if debug_flag set
 */
 ...
 *----------------------------------------------------------------------------*/
    ...
    register struct bbk_reg	*reg;		/* device registers */
...
/*============================================================================*/
     ...
     * Initialisation
     */
    reg = BBK_BASEADDR(dev);
    debug ("bbkinit () - reg = 0x%x\n", reg);
    if ((int)reg < 0 || (int)reg > 0x0FC0)
    {	/* base register address out of range */
	return ENXIO;
    }

    unit = BBK_UNITNUM (dev);

    bbk = &bbkdevice[unit];

    debug ( "bbkinit () - bbk%d: init - addr = %x, irq = %d\n", 
		unit, reg, BBK_INTCHAN(dev) );

    bbk->int_chan = BBK_INTCHAN(dev);
    if (bbk->int_chan < 2 || bbk->int_chan > 7)
    {	/* irq out of range */
	return ENXIO;
    }
    ...
    {
	debug( "bbkinit () - bbk%d: C012 ISR check failed (isr = %x);  disabling device.\n",
    ...
    {
	debug( "bbkinit () - bbk%d: C012 OSR check failed( osr = %x);  disabling device.\n",
...
/*============================================================================*/
    ...
    debug ("bbkopen () - dev = 0x%x (%d)\n", dev, dev);

    unit = BBK_UNITNUM( dev );

    debug ("bbkopen () - unit = %d\n", unit);
    ...
    debug ( "bbkopen () - bbk%d: init - addr = %x, irq = %d\n", 
		unit, UNIT_TO_REG(unit), BBK_INTCHAN(dev) );
    ...
    /*
     * Initially and abort on errors clear
     */
...
/* Local Variables */
    ...
    int			unit;		/* unit number */
    struct bbk_reg	*reg;		/* device structure pointer */
...
/* Local Variables */
    ...
    register struct bbk_reg	*reg;		/* B008 reegisters */
...
/* Local Variables */
    ...
    register struct bbk_reg	*reg;		/* B008 registers */
    register struct bbk_device	*bbk;		/* device structure */
    register int		unit;		/* unit number */
...
/*============================================================================*/
     ...
     * Initialisation
     */
    unit = BBK_UNITNUM( bp->b_dev );
    bbk = &bbkdevice[unit];
    reg = BBK_BASEADDR (bp->b_dev);
...
/* Local Variables */
    ...
    register int		unit;	/* unit number */
    register struct bbk_reg	*reg;	/* register pointer */
    register struct bbk_device	*bbk;	/* device structure pointer */
...
/*============================================================================*/
    ...
    /*
     * Extract unit number from argument (unit << 16 before timeout () is
     * called.
     */
    unit = arg >> 16;
    /*
