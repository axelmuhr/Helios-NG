 ...
 *
 *	bits 0,1	Fifo address, where 0 is 0xC0000
 *					    1 is 0xC8000
 *					    2 is 0xD0000
 *					    3 is 0xD0000
 *    
 ...
 */
#define debug4		if(debug_flag&4)printf
#define debug2		if(debug_flag&2)printf
#define debug1		if(debug_flag&1)printf
...
static struct hepc_reg *board_address[4] = {
...
static struct hepc_device
{
    ...
    u_char *	addr_pif;	/* performance interface (fifo) address */
...
/*============================================================================*/
    ...
    hepc->addr_pif = fifo_address[HEPC_PIF(dev)];
    /*
     * Reset the device
     */
...
/*============================================================================*/
	...
	{
	    debug1("hepc%d: Open (2) by process %d\n", unit, u.u_procp->p_pid );
	    return;
	}
	else
	{
    ...
    debug1( "hepc%d: Open (1) by process %d\n", unit, u.u_procp->p_pid );
    /*
...
/*============================================================================*/
	...
	else
	{
	     ...
	     */
	    debug4("hepc%d:  link adaptor i/o - %d\n", unit, hepc->read_count );
        ...
        if ( hepc->read_abort != 0 )
        {  
            debug4( "hepc%d:  read aborted (%x), resid = %d\n",
	...
	else
	{
	    ...
	    debug4( "hepc%d:  link adaptor i/o\n", unit );
        ...
        if ( hepc->write_abort != 0 )
        {
            debug4( "hepc%d:  write aborted (%x), resid = %d\n",
    ...
    debug4( "hepc%d: i/o done, resid = %d\n", unit, bp->b_resid );
...
/* Local Variables */
    ...
    u_char *		addr_pif;	/* address of pif */
...
/*============================================================================*/
	...
	if(hepc->read_count > MAXBLOCK)
	    {
#if 0
	    repinsb(addr_pif, cptr, MAXBLOCK);
#endif
	    bcopy(addr_pif, cptr, MAXBLOCK);
	    hepc->read_offset += MAXBLOCK;
	    hepc->read_count -= MAXBLOCK;
	    }
	else
	    {
#if 0
	    repinsb(addr_pif, cptr, hepc->read_count);
#endif
	    bcopy(addr_pif, cptr, hepc->read_count);
	    hepc->read_offset += hepc->read_count;
	    hepc->read_count = 0;
	    }
	break;
...
debug1("copy %d bytes from %x to %x\n", hepc->write_count, cptr, addr_pif);
	if(hepc->write_count > MAXBLOCK)
	    {
#if 0
	    repoutsb(addr_pif, cptr, MAXBLOCK);
#endif
	    bcopy(cptr, addr_pif, MAXBLOCK);
	    hepc->write_offset += MAXBLOCK;
	    hepc->write_count -= MAXBLOCK;
	    }
	else
	    {
#if 0
	    repoutsb(addr_pif, cptr, hepc->write_count);
#endif
	    bcopy(cptr, addr_pif, hepc->write_count);
	    hepc->write_offset += hepc->write_count;
	    hepc->write_count = 0;
	    }
	break;
    }

    if(READREG(reg->pifsr) & LINK_READY)
	debug1("timeout from hepc board\n");
    return;
}
...
/*============================================================================*/
    ...
    debug4( "hepc%d:  ioctl %x, status = %x\n", 
	...
	break;
      ...
      case HEPC_TIMEOUT:
        ...
        debug1( "hepc%d:  timeout set to %d ticks\n", unit, hepc->timeout );
        break;
	 ...
	 */
	debug4( "hepc%d:  ISR at %x contains %x\n", 
	...
	break;
      ...
      case HEPC_OUTPUT_READY:
         ...
         */
        debug4( "hepc%d:  OSR at %x contains %x\n",
	...
	debug1( "hepc%d:  Bad ioctl command 0x%x\n", unit, cmd );
	u.u_error = ENOTTY;
	return;
    }
...
/*============================================================================*/
        ...
        if ( hepc->int_chan == irq )
	{
	     ...
	     */
	    if ( (addr_intsr & INT_RECEIVE_READY) &&
		 ( READREG( addr_isr ) & LINK_READY ) )
	    {
		debug2( "hepcintr:  read interrupt\n" );
	     ...
	     */
	    if ( (addr_intsr & INT_SEND_READY) &&
		 ( READREG( addr_osr ) & LINK_READY ) )
	    {
		debug2( "hepcintr:  write interrupt\n" );
