...
#define MAXREAD		(5*1024)
#define MAXWRITE	(8*1024)

int	maxread		= MAXREAD;
int	maxwrite	= MAXWRITE;
...
static void nfs_read(nfsrw *ar)
{
	Wait(&nfslock);
	nfsclnt->cl_auth = ar->auth;
	while(clnt_call(nfsclnt, NFSPROC_READ, xdr_readargs, &ar->arg.r, xdr_readres, &ar->res.r, TIMEOUT ) != RPC_SUCCESS )
	{
...
static void nfs_write(nfsrw *ar)
{
	Wait(&nfslock);
	nfsclnt->cl_auth = ar->auth;	
	while(clnt_call(nfsclnt, NFSPROC_WRITE, xdr_writeargs, &ar->arg.w, xdr_attrstat, &ar->res.w, TIMEOUT ) != RPC_SUCCESS )
	{
...
static void do_read(MCB *mcb, nfshandle *h, nfsrw *ar)
{
	...
	while( sent < size )
	{
		...
		{
			ErrorMsg(mcb,EC_Error | EG_Errno | (word)nfs_errno(ar->res.r.status));
			return;
		}
...
static void do_write(MCB *mcb, nfshandle *h, nfsrw *ar)
{
	...
	while( got < size )
	{
		...
		if( ar->res.w.status != NFS_OK )
		{
			...
			e = EC_Error|EG_Errno| (word)nfs_errno(ar->res.r.status);
			break;
		}
...
extern void file_server(MCB *mcb, nfshandle *h, AUTH *auth)
{
	...
	nfsrw *ar = (nfsrw *)Malloc(sizeof(nfsrw));
