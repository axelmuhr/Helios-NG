...
/*$Log: tftp.c,v $
 * Revision 1.2  1991/04/16  11:14:54  nickc
 * changed define dependencies from helios to __HELIOS
 *
 * Revision 1.1  1991/04/16  10:08:46  nickc
 * Initial revision
 *
 */
...
send_ack:
		...
		if (sendto(f, ackbuf, size, 0, (caddr_t)&sin,
		    sizeof (sin)) != size)
		{
/*			IOdebug( "tftp : send failed, errno = %d",
				errno ); */
			alarm(0);
			perror("tftp: sendto");
			goto abort;
...
/*				IOdebug( "tftp : receive failed" ); */
	...
	/*      size = write(fd, dp->th_data, n - 4); */
		...
		if (size < 0)
		{
/*			IOdebug( "tftp : failed to write file" ); */
			nak(errno + 100);
			break;
		}
