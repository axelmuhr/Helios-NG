...
/* I worry a little about signed zeros here. I hope that -0.0 == 0.0     */
    ...
    d1.d = d;
    n = 4*(d1.i.x - 0x40);      /* Check that I have the offset right here */
    d1.i.x = 0x40;
    while (d1.d < 0.5) d1.d += d1.d, n--;
    *lvn = n;
    return d1.d;
}
...
double d; int n;
{
    ...
    n &= 3;
    if ((mhi & 0x00c00000)==0)
        if ((mhi & 0x00200000)==0) nx1 = n - 3;
        else nx1 = n - 2;
    else if (!(mhi & 0x00800000)) nx1 = n - 1;
    else nx1 = n;
    if (nx1 > 0) nx++, n -= 4;
...
/* I now know what the true exponent in the result will be.              */
...
/* terminate.                                                            */
        ...
        {   flag = d1.i.mhi & 0x00080000;
            if (d1.i.mlo & 0x80000000) d1.i.mhi = (d1.i.mhi << 1) | 1;
            else d1.i.mhi = d1.i.mhi << 1;
            d1.i.mlo = d1.i.mlo << 1;
            n--;
        } while (flag==0);
        *lvn = n;
...
#if defined(ARM) && !defined(SOFTWARE_FLOATING_POINT)

/* The ARM has a load of trig functions etc available as opcodes, so     */
/* has no need for the software versions shown here.                     */
/* But of course with software floating point this does not apply!       */
...
double atan(double x)
{
    int n;
    double f;
    const static double a3[4] = { 0.0, _pi_6, _pi_2, _pi_3 };
    f = fabs(x);
    if (f > 1.0)
    {   f = 1.0 / f;
        n = 2;
...
double _asinacos(double x, int flag)
{
    int i;
    double y, g, r;
    const static double a4[2] = {0.0, _pi_4 };
    const static double b1[2] = {_pi_2, _pi_4 };
    y = fabs(x);
    if (y < 1.e-10) i = flag;
    else
    {   if (y > 0.5)
