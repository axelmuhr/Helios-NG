 ...
 */
  ...
  []-------------------------------------------------------------------------[]
   ...
   |                                                                         |
   |  History:                                                               |
   |    2 - O.Imbusch - 14 May   1991 - Error handling centralized           |
 ...
 *
 * Parameter  : - nothing -
 * Return     : FALSE if any fatal error occurres
 *		TRUE  if ok
 *
 *************************************************************************/
word
handle_dups_and_links ( void )
{
 ...
 bpar = bread ( cdev, 1, 1, SAVEA );	/* Get the summary-block with	*/
 if (!bpar) {
 	Report ("%shandle_dups_and_links() : Failed to read summary block !", S_FATAL);
	return (FALSE); 	
 }
 ...
 tpt	= &head;			/* element is the only one.	*/
  ...
  Report ("%sTraversing the dir-tree a second time", S_INFO);

  if ( fst.dup_blocks )
    Report ("%s - LOOKING FOR MULTIPLE ALLOCATED BLOCKS", S_INFO);

  if ( found_links )
    Report ("%s - LOOKING FOR SYMBOLIC LINK REFERENCES", S_INFO);
 ...
 for ( ;; )
 {
	...
	if ( de_pt != (struct dir_elem *) NULL )
	{
		...
		if ( ! silent_mode )
			Report ("%sActual object = %s / [%s]", S_INFO,
				    path, de_pt->de_name);
...
/*-------  The validating operations to be done with this entry : ------*/
		...
		{			/* links found ...		*/
			...
			{
				Report ("%sThe entry '%s' is referred by %d symbolic links",
...
DEBdebug ("	handle_dups_and_links : Have examined all entries. Change the directory");
...
DEBdebug ("	handle_dups_and_links : Take last sub-dir as parent directory");
...
DEBdebug ("	handle_dups_and_links : Use backtracking to get a new parent-dir");
...
DEBdebug ("	handle_dups_and_links : Backtracking to path = %s", path );
...
DEBdebug ("	entry_dup_bnr :	Check this entry for multiple allocated blocks.");
...
DEBdebug ("	entry_dup_bnr :	Scan the direct block table.");
...
DEBdebug ("	entry_dup_bnr : Scan the single indirect block numbers.");
	...
	bip1 = bread ( cdev, de->de_inode.i_ib[0], 1, SAVEA );
	if (!bip1)
        {
          Report ("%sentry_dup_bnr() : Failed to read indirect block (bnr %d) !",S_FATAL,de->de_inode.i_ib[0]);
...
DEBdebug ("	entry_dup_bnr : Scan the double indirect block numbers.");
	...
	{				/* The parent inode block and	*/
		...
		append_dup_entry ( de->de_inode.i_ib[1], de->de_name,
				   ...
				   &rf );
					...
					/* --> Look above !		*/
					/* Read the indirect block	*/
	bip2 = bread ( cdev, de->de_inode.i_ib[1], 1, SAVEA );
	if (!bip2) {
Report ("%sentry_dup_bnr() : Failed to read indirect block (bnr %d) !",S_FATAL,de->de_inode.i_ib[1]);
		longjmp (term_jmp, IND_BLK_ERR);
	}
					/* Scan the block content	*/
	for ( bcnt = 0 ; bcnt < i_fs.fs_maxcontig ; bcnt++ )
	{				/* Skip over zero block numbers	*/
		...
		bip1 = bread ( cdev, bip2->b_un.b_daddr[bcnt], 1, SAVEA );
		if (!bip1) {
Report ("%sentry_dup_bnr() : Failed to read double indirect block (bnr %d) !",S_FATAL,bip2->b_un.b_daddr[bcnt]);
		...
		for ( bcnt_d = 0 ; bcnt_d < i_fs.fs_maxcontig ; bcnt_d++ )
		{
			if ( ! bip1->b_un.b_daddr[bcnt_d] )
				continue;
					...
					/* Look for multiple allocations*/
			if ( bitmap_get ( bip1->b_un.b_daddr[bcnt_d] ) > 1 )
			{		/* Prepare reference information*/
				rf.ind_bnr[0] = bip2->b_un.b_daddr[bcnt];
				rf.ind_off[0] = bcnt;
...
DEBdebug ("	decide_on_dups :	Examine the entries which refer a dup block.");
...
/*---------  Examine the entries which refer to the dup block  ---------*/
		...
		ept = hpt->dupenxt;	/* Initialization to access the */
					...
					/* Print a report		*/
		Report ("%sMultiple referred block = %d | ref-count = %d | claimed by :",
		...
		if ( hpt->dupenxt == (struct dup_entry *) NULL )
		{
			...
			Report ("%sThe block is claimed by no entry!",
				  S_WARNING);
		...
		{
			case POSSIBLE_DIR :
				Report ("%sThe block %d seems to be of type DIRECTORY",
					    S_INFO, hpt->bnr );
				break;
			case POSSIBLE_INDIRECT :
				Report ("%sThe block %d seems to be an indirect block.",
					    S_INFO, hpt->bnr );
				break;
			case POSSIBLE_UNKNOWN :
				Report ("%sThe block %d seems to be a data-block",
					    S_INFO, hpt->bnr );
				break;
		}
...
DEBdebug ("	decide_on_dups :	Note : The entry [%s] is not regarded",
	 ept->name );
...
DEBdebug ("	decide_on_dups :	No 'usable' entry found");
...
DEBdebug ("	decide_on_dups :	There is the only one entry");
...
DEBdebug ("	dup_select_entry :	No 'usable' entry is available");
...
DEBdebug ("	dup_select_entry :	Find the single entry marked as 'usable'");
...
DEBdebug ("	dup_select_entry :	The entry with the most recent date is chosen.");
...
DEBdebug ("	dup_select_entry :	The entry '%s' gets the block",
	 ename );
...
DEBdebug ("	dup_select_entry :	Delete reference in entry : %s (ref-lev=%d)",
	 ept->name, ept->b_ref.ref_level );

					/* Read the specified directory- */
					/* block into memory and ...	 */
	bp = bread ( cdev, ept->b_ref.pi_bnr, 1, SAVEA );
	if (!bp)
        {
          Report ("%sdup_select_entry() : Failed to read directory block (bnr %d) !",S_FATAL,ept->b_ref.pi_bnr);
...
DEBdebug ("	dup_select_entry :	Dup block on inode-level. (place=%d)",
	 ept->b_ref.pi_place );
...
DEBdebug ("	dup_select_entry :	Dup block on single indirect level. (offset=%d)",
	 ept->b_ref.ind_off[0] );

					/* Read the indirection block	 */
			bind = bread ( cdev, ept->b_ref.ind_bnr[0], 1, SAVEA );
			if (!bind) {
Report ("%sdup_select_entry() : Failed to read indirect block (bnr %d) !",S_FATAL,ept->b_ref.ind_bnr[0]);
				longjmp (term_jmp, IND_BLK_ERR);
			}
...
DEBdebug ("	dup_select_entry :	Dup block on double indirect level. (offset=%d)",
	 ept->b_ref.ind_off[1] );

					/* Read the indirection block	 */
			bind = bread ( cdev, ept->b_ref.ind_bnr[1], 1, SAVEA );
			if (!bind) {
Report ("%sdup_select_entry() : Failed to read indirect block (bnr %d) !",S_FATAL,ept->b_ref.ind_bnr[1]);
				longjmp (term_jmp, IND_BLK_ERR);
			}
					/* Clear the reference and ...	 */
...
DEBdebug ("	dup_select_entry :	Update inode for %s on disk",
	 ept->name );
...
append_dup_bnr ( daddr_t bnr, word refcnt )
{
 ...
 Report ("%sBlock no %d is multiple allocated.", S_WARNING, bnr );


DEBdebug ("	append_dup_bnr :	Append block %d to hash table on slot %d",
	 bnr, index);
...
DEBdebug ("%sappend_dup_bnr() : Unable to allocate memory !", S_FATAL);
 ...
 bp = bread ( cdev, bnr, 1, SAVEA );	/* Get the block and examine	*/
					/* it's content:		*/
 if (!bp) {
Report ("%sappend_dup_bnr() : Failed to read block (bnr %d) !",S_FATAL,bnr);
...
DEBdebug ("	append_dup_bnr :	This block seems to be a directory-block.");
...
DEBdebug ("	append_dup_bnr :	This block seems to be an indirect block.");
...
DEBdebug ("	append_dup_bnr :	The block is neither a directory nor an indirect block");
		   ...
		   word type, struct ref *r )
{
 ...
 if ( hpt->bnr == dupbnr )
 {
	Report ("%sEntry '%s' refers multiple allocated block no %d",
		    S_INFO, name, dupbnr );
...
DEBdebug ("%sappend_dup_entry() : Unable to allocate memory !", S_FATAL);
...
DEBdebug ("	append_dup_entry :	Matching block type and entry mode-flag");
...
DEBdebug ("	append_dup_entry :	The type of the block and the mode-flag divert");
...
DEBdebug ("	remove_dup_hash :	Clear duplicate bnr hash-table.");
