...
/*----------  Pass 1 : Look for freed and new allocated blocks  ----------*/
 ...
 IOdebug ("%s***    Step 3.1 : Look for freed or allocated blocks.", S_INFO);
	...
	{				/* Work on the actual bit-map	  */
		...
		{				
			...
			if ( ( bit_maps[cgnr][bcnt] & MASK_REFCNT ) == 0 )
			{
				...
				{
#if GEPDEBUG
IOdebug(" check_blocks() : free block %d, cylinder group %d",bcnt,cgnr);
#endif
...
#if DEBUG
IOdebug (">     usable : cg = %d  :  FREE  sum = %d | count = %d",
...
#if DEBUG
IOdebug ("> NOT usable : cg = %d  :  FREE  sum = %d | count = %d",
  	cgnr, bp->b_un.b_info->cgx.cg_s.s_nbfree, n_free);
#endif
 	...
 	if ( changes )			
 	{
		IOdebug ("%sUpdate modified info-block cg = %d on disk.",
			 S_INFO, cgnr);
	 	test_bwrite ( bp );
	}
...
/*-------------  Pass 2 : Look for "lost" directory-blocks  --------------*/
 ...
 IOdebug ("%s***    Step 3.2 : Look for 'lost' directory blocks", S_INFO);
...
#if DEBUG
IOdebug ("	check_blocks :	Block no %d is a 'lost' block", bnr );
#endif
 				...
 				{
#if DEBUG
IOdebug ("	check_blocks :	The 'lost' block no %d contains directory information",
	 bnr );
#endif
 		...
 		if ( changes )
 		{
 			IOdebug ("%sUpdate info-block of cg %d on disk", 
 				  S_INFO, cgnr);
 			test_bwrite ( bp );
 		}
 		else
...
#if DEBUG
IOdebug ("	check_blocks :	Look for the /lost+found directory");
#endif

 bp = bread ( 0, 1, 1, SAVEA );		/* Get the summary block	  */
 ...
 {					/* /lost+found is not available !! */
	...
	{				/* new /lost+found inode.	  */
		IOdebug ("%sNo /lost+found directory available!", S_WARNING);
		brelse ( bp->b_tbp, TAIL );
 		lost_found = FALSE;
 	}
 }
...
#if DEBUG
IOdebug ("	check_blocks :	Handle lost directory-blocks which are 'local roots'");
#endif
...
#if DEBUG
IOdebug ("	check_blocks :	Take block no %d as a 'local root block'",
	 lpt->bnr );
#endif
				...
				{	
#if DEBUG
IOdebug ("	check_blocks :	Create an entry for the block (valid=%d)",
	 val_entries );
#endif
				...
				{
#if DEBUG
IOdebug ("	check_blocks :	The block does not keep enough valid entries.");
#endif
				}
 ...
 {
#if DEBUG
IOdebug ("	check_blocks :	The /lost+found-inode has to be corrected");
#endif
 	bp = bread ( 0, 1, 1, SAVEA );	/* Get the summary-block	  */
...
/*----  Pass 3 : Look for multiple allocated blocks and handle links  ----*/
 ...
 IOdebug ("%s***    Step 3.3 : Look for multiple referred blocks.", S_INFO);
 ...
 {					/* Deal with each block number in */
 	...
 	for ( bcnt = 0 ; bcnt < i_fs.fs_cgsize ; bcnt++ )
 	{
 		...
 		{
#if DEBUG
IOdebug ("	check_blocks :	Block No %d is multiple allocated.", 
 ...
 {				/* the corresponding entries		  */
	...
	{				/* the root inode.		  */
#if DEBUG
IOdebug ("	check_blocks :	Found %d multiple allocated blocks",
	 fst.dup_blocks );
#endif
					/* Get the summary - block	  */
		bp = bread ( 0, 1, 1, SAVEA );	
 ...
 /* 
 				handle_dups_and_links ();
 */
 			IOdebug ("%s'Hanging' links are ignored.", S_WARNING);
 			ignore_links = TRUE;
...
#if DEBUG
IOdebug ("	slot_available :	Look for a free /lost+found-slot");
#endif
 ...
 for ( dbcnt = 0 ; dbcnt < i_fs.fs_ndaddr ; dbcnt++ )
 {
 	...
 	{
#if DEBUG
IOdebug ("	slot_available :	The %d. direct block keeps no reference",
	 dbcnt );
#endif
 		return TRUE;
 	}
 	...
 	bp = bread ( 0, lfpt->i_db[dbcnt], 1, SAVEA );
 	...
 	{				/* An empty slot ?		 */
 		...
 		{
#if DEBUG
IOdebug ("	slot_available :	Found a free slot on pos %d in the %d. direct block",
	 offcnt, dbcnt );
#endif
 			brelse ( bp->b_tbp, TAIL );
 			return TRUE;
 ...
 IOdebug ("%sThe /lost+found-directory is full. All other blocks are lost!", S_WARNING);
 return FALSE;
}
...
#if DEBUG
IOdebug ("	get_free_slot :	Search an unused slot in an allocated direct blocks");
#endif
 ...
 for ( dbcnt = 0 ; dbcnt < i_fs.fs_ndaddr ; dbcnt++ )
 {
 	...
 	{				/* ... previously allocated:	 */
 		...
 		bp = bread ( 0, ip->i_db[dbcnt], 1, SAVEA );
 		...
 		{			/* Look for an empty slot	 */
 			...
 			{		
#if DEBUG
IOdebug ("	get_free_slot :	Free slot found the %d. direct block (bnr=%d) at pos %d",
...
#if DEBUG
IOdebug ("	get_free_slot :	Allocate a new block and search an unused slot.");
#endif 
 ...
 for ( dbcnt = 0 ; dbcnt < i_fs.fs_ndaddr ; dbcnt++ )
 {
 	...
 	{				/* Allocate the first available	 */
		...
		new_bnr = new_alloc ();
 		...
 		{
			 IOdebug ("%sNo more free blocks available!", S_WARNING);
 			 return FALSE;
 		}
#if DEBUG
 		else
 			 IOdebug ("%sBlock no %d is allocated.", S_INFO, new_bnr);
#endif
		 
#if DEBUG
IOdebug ("	get_free_slot :	Block no %d is noted in the directory on pos %d",
	 new_bnr, dbcnt );
#endif
 		...
 		for ( offcnt = 0 ; offcnt < i_fs.fs_maxdpb ; offcnt++ )
 		{
 			...
 			{		
#if DEBUG
IOdebug ("	get_free_slot :	Found a free slot in the %d. direct block at pos %d",
	 dbcnt, offcnt );
#endif
 					/* Save block number and offset  */
				*bnr = ip->i_db[dbcnt];
		...
		brelse ( bp->b_tbp, TAIL );
					...
					/* code - region		 */
 IOdebug ("%sThe specified directory is full!", S_WARNING);
 return FALSE;
}
...
#if DEBUG
IOdebug ("	new_alloc :	Allocate a new block for the /lost+found - dir");
#endif
					/* Deal with each cylinder-group */
 for ( cgnr = 0 ; cgnr < i_fs.fs_ncg ; cgnr++ )
 {
 					/* Get the info-block		 */
 	bip = bread ( 0, info_blocks[cgnr], 1, SAVEA );
 	...
 	{				
		...
		{			
#if DEBUG
IOdebug ("	new_alloc :	Block no %d is free and now allocated",
	cgnr * i_fs.fs_cgsize + bcnt );
#endif
					/* Note the allocation:		 */
			...
			test_bwrite ( bip );
					/* Read the "new" block ,	 */
			bp = bread ( 0, cgnr*i_fs.fs_cgsize+bcnt, 1, SAVEA );
...
#if GEPDEBUG
IOdebug ("	workon_lost_block :	Validate the entries in 'lost' block no %d",
	 lbnr );
#endif
...
/*-------  We have to validate each entry and to look for sub-dirs  ------*/ 
	...
	{
#if GEPDEBUG
IOdebug ("	workon_lost_block :	A valid entry '%s' is found on slot %d",
	 &bp->b_un.b_dir[icnt].de_name, icnt );
		...
		if ( bp->b_un.b_dir[icnt].de_inode.i_mode == Type_Directory )
		{
			...
			IOdebug ("%s'%s' is a Subdirectory and will be used as 'local' root",
			...
			{
#if GEPDEBUG
IOdebug ("	workon_lost_block :	Write 'lost' block temporarily on disk");
#endif
				test_bwrite ( bp );
			}
			else		
			{
#if GEPDEBUG
IOdebug ("	workon_lost_block :	Release 'lost' block");
#endif
				brelse ( bp->b_tbp, TAIL );
			}
...
#if GEPDEBUG
IOdebug ("	workon_lost_block :	Read 'lost' block %d again",lbnr);
#endif
			bp = bread ( 0, lbnr, 1, SAVEA );
		}
	}
 ...
 {					/* the entries in the lost block: */
 	IOdebug ("%sUpdate 'lost' directory-block finally on disk\n", S_INFO);
 	test_bwrite ( bp );
 }
 else
 	brelse ( bp->b_tbp, TAIL );
...
#if DEBUG
IOdebug ("	create_lost_entry :	Create an entry for block %d in %d at position %d",
	 lostbnr, lfbnr, dbslot );
#endif
					/* Get the specified block from	*/
 blfp = bread ( 0, lfbnr, 1, SAVEA);	/* the /lost+found-directory.	*/
 ...
 IOdebug ("%sUpdate modified /lost+found directory block on disk", S_INFO);	
...
#if DEBUG
IOdebug ("	poss_lost_dir_block :	Test block no %d for 'lost directory block'",
	 bnr );
#endif
					/* Clear the comparison struct  */
 memset ( &de, 0, sizeof (struct dir_elem) );
					...
					/* DIRECTORY !			*/
 bp = bread ( 0, bnr, 1, SAVEA );	/* Read the block into cache	*/
 ...
 {					/* block, if it is of type-dir	*/
 					...
 					/* try kept in it.		*/
#if DEBUG
IOdebug ("	poss_lost_dir_block :	The block is NOT of type directory and will be ignored");
#endif
	brelse ( bp->b_tbp, TAIL );		
 	return FALSE;
 }
...
#if DEBUG
IOdebug ("	poss_lost_dir_block :	'%s' claims %d dup blocks and will be deleted",
 ...
 {
 	IOdebug ("%sWrite modified 'lost' directory-block to disk", S_INFO);
 	test_bwrite ( bp );
 }
 else
 	brelse ( bp->b_tbp, TAIL );
 ...
 {					/* entry to succeed.		*/
#if DEBUG
IOdebug ("	poss_lost_dir_block :	Found %d usable entries in the lost dir-block",
	 valid_entries );
#endif
 	return TRUE;		
 }
 ...
 {
#if DEBUG
IOdebug ("	poss_lost_dir_block :	The block is not a usable 'lost directory block'"); 
#endif
 	return FALSE;
 }
}
...
#if DEBUG
IOdebug ("	search_local_roots :	Find 'local roots' by comparison of the hash-tabs");
#endif
 	...
 	while ( lpt != (struct lost_bnr *) NULL )
 	{
		...
		{			
			...
			{		/* We can't use the block-number*/
					/* as a local root !		*/
#if DEBUG
IOdebug ("	search_local_roots :	Block no %d is NOT a 'local root block'",
	 lpt->bnr );
#endif					/* Note that the block is not   */
		...
		{		
			...
			IOdebug ("%sBlock no %d is a non-referred 'local root block' !",
	 			    S_INFO, lpt->bnr );
		}
...
#if DEBUG
IOdebug ("	append_lost_bnr :	Append lost block no %d to hash-table on slot %d",
	 lbnr, index );
#endif
 ...
 {
#if DEBUG
IOdebug ("	append_lost_bnr :	Unable to allocate memory !");
#endif
 	longjmp (term_jmp, 1);
 }
...
#if DEBUG
IOdebug ("	remove_lost_hash :	Free 'lost' block number hash-table");
#endif 
...
#if DEBUG
IOdebug ("	append_ref_bnr :	Examine references kept in block %d",
	 lbnr );
#endif 

 bp = bread ( 0, lbnr, 1, SAVEA );	/* Get the lost block		*/
...
#if DEBUG
IOdebug ("	append_ref_bnr :	Inspect the %d. inode held in the block",
	 icnt );
#endif
 	...
 	{				/* Take the next direct block-  */
 		...
 		if ( ! valid_bnr ( dp->de_inode.i_db[dbcnt] ) )
 			continue;
#if DEBUG
IOdebug ("	append_ref_bnr :	The %d. direct block refers a valid block",
	 dbcnt );
#endif
 					/* Calculate the hash-index	*/
		...
		{			/* The entry is contained in the*/
			...
			{
#if DEBUG
IOdebug ("	append_ref_bnr :	Reference to block %d found in the hash-table",
...
#if DEBUG
IOdebug ("	append_ref_bnr :	Append reference to hash-table on slot %d",
	 index );
#endif
					/* Allocate memory for the NEW	*/
					/* entry			*/
		...
		{
#if DEBUG
IOdebug ("	append_ref_bnr :	Unable to allocate memory !");
#endif
			longjmp (term_jmp, 1);
		}
...
#if DEBUG
IOdebug ("	remove_ref_hash :	Free lost block reference hash-table");
#endif 
					/* Work on each row of the hash- */
					/* table			 */
