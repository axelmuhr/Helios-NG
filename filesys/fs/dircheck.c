...
#if DEBUG
IOdebug ("	validate_entry : Working on [%s]", dp->de_name );
#endif
...
#if DEBUG
IOdebug ("	validate_entry :	Check the entry name syntactically.");
#endif
 ...
 if ( valid_entry_name ( dp->de_name , TRUE ) )
 {
					...
					/* cally correct.		*/
#if DEBUG
IOdebug ("	validate_entry :	Look for duplicate entry name.");
#endif
...
#if DEBUG
IOdebug ("	validate_entry :	Check the time-stamps for zero.");
#endif
 ...
 {					/* Non-plausibel time-stamps	*/
					...
					/* all error counter.		*/
#if DEBUG
IOdebug ("	validate_entry :	Check the time-stamps for plausibility.");
#endif
...
#if DEBUG
IOdebug ("	validate_entry :	Check the access matrix");
#endif
...
#if DEBUG
IOdebug ("	validate_entry :	Check # of blocks and entry size.");
#endif
...
#if DEBUG
IOdebug ("	validate_entry :	Check the validity of the block-numbers.");
#endif
...
#if DEBUG
IOdebug ("	validate_entry :	Check the type of the entry");
#endif
 ...
 switch ( dp->de_inode.i_mode )
 {
	case Type_Directory :		/* Directory entry-mode		*/
				...
				{	/* A non-zero reference	?	*/
					...
					/* Read a directory-block	*/
						bp = bread ( 0, dp->de_inode.i_db[bcnt], 1, SAVEA );
	...
	case Type_Link	    :		/* Symbolic link entry-mode	*/
				...
				if ( dp->de_inode.i_db[0] )
				{
					...
					bp = bread ( 0, dp->de_inode.i_db[0], 1, SAVEA );
 ...
 {
	if ( de_errors == 1 )
		IOdebug ("%s1 error was detected for this entry:", S_SERIOUS);
	else
		IOdebug ("%s%d errors were detected for this entry:", 
			 S_SERIOUS, de_errors);

	if ( mode_err )
		IOdebug ("%s: The type inspection has failed. (type-flag found: 0x%x)\n",
			 S_INFO, dp->de_inode.i_mode);
	if ( dup_name_err )
		IOdebug ("%s: Entry name found twice in the same directory.\n",
			 S_INFO);
	if ( times_err & SERIOUS )
		IOdebug ("%s: The time-stamps of the entry are damaged.\n", S_INFO);
	elif ( times_err & WARNING )
		IOdebug ("%s: Relations between the time-stamps are bad.\n", S_INFO);
	if ( matrix_err )
		IOdebug ("%s: The access matrix is corrupted.\n", S_INFO);
	if ( blocks_err )
		IOdebug ("%s: The block-count for the entry is totally wrong.\n", S_INFO);
	if ( size_err )
	{
		IOdebug ("%s: The size-field of the entry (%d) is suspicious:", 
			  S_INFO, dp->de_inode.i_size);
		IOdebug ("%s  it should be between %d and %d\n", 
			  S_INFO, dp->de_inode.i_blocks * i_fs.fs_bsize, (dp->de_inode.i_blocks + 1) * i_fs.fs_bsize );
	}
	if ( inval_cnt )
		IOdebug ("%s: Found %d invalid direct block number references.\n",
			S_INFO, inval_cnt);
}
      ...
      <= inval_cnt )			/* Yes !!! - That's bad...	*/
 {
	fatal = TRUE;
	fst.mingood_dir++;
	IOdebug ("%sToo many invalid block numbers (%d) found !",
		    S_SERIOUS, inval_cnt);
 }
 ...
 {
	fatal = TRUE;
	IOdebug ("%sToo many inode-errors found !",
		    S_SERIOUS, de_errors);
...
/*-----------------  Correct the mode of the entry  --------------------*/
 ...
 if ( mode_err )			/* it is corrupted.		*/
 {
	if ( should_be == Type_Directory && my_strcmp(path, "/") ) {
		...
		IOdebug ("%sI try to find out the correct type of the entry.",
		 	   S_SERIOUS);
...
/*-------------------  Correct the name of the entry  ------------------*/
 ...
 if ( name_err || dup_name_err )	/* ted: type in a new one !	*/
 {
	...
	{				/* Try it again and again ...	*/
		if ( name_err )
			IOdebug ("%sThe entry [%-32s] has an invalid name.",
				  S_SERIOUS, dp->de_name);
		elif ( dup_name_err )
			IOdebug ("%sThe entry [%-32s] has a duplicate name.",
				  S_SERIOUS, dp->de_name);
		...
		IOdebug ("%sGenerated a new entry name : '%s'.", S_INFO, in_buf );
...
/*---------------------  Correct the time-stamps  ----------------------*/
 ...
 if ( times_err )
 {
	...
	if ( dp->de_inode.i_ctime == 0 )
	{
		...
		IOdebug ("%sThe creation time-stamp is corrected.", S_INFO);
		changes_de++;		/* Note the modification	*/
	}
	...
	if ( dp->de_inode.i_ctime > dp->de_inode.i_mtime )
	{
		...
		IOdebug ("%sThe modification time-stamp is corrected.", S_INFO);
		changes_de++;
	}
	...
	if ( dp->de_inode.i_mtime > dp->de_inode.i_atime )
	{
		...
		IOdebug ("%sThe access time-stamp is corrected.", S_INFO);
		changes_de++;
	}
 }
...
/*--------------------	Allow access to the entry  ---------------------*/
 ...
 if ( matrix_err )
 {
	IOdebug ("%sThe default 'da' permissions are added to the matrix.", S_INFO);
	switch ( dp->de_inode.i_mode )
	{
...
#if DEBUG
IOdebug ("	validate_entry :	Work on the allocation scheme.");
#endif
 ...
 {					/* If we have found any block-	*/
	...
	{
#if DEBUG
IOdebug ("	validate_entry :	Note at pos %d block number %d",
...
#if DEBUG
IOdebug ("	validate_entry :	Found free slot at pos %d", bcnt );
#endif
 ...
 for ( bcnt = 0 ; bcnt < i_fs.fs_niaddr ; bcnt++ )
 {
	...
	{				/* Multiple allocated ?		*/
		if ( bitmap_get (dp->de_inode.i_ib[bcnt]) > 0 )
		{
			...
			/* it.						*/
#if DEBUG
IOdebug ("	validate_entry :	Indirection block (level=%d) is multiple allocated.",
	 bcnt + 1 );
#endif
			if ( ref_update )
			{
		...
		{
#if DEBUG
IOdebug ("	validate_entry :	Inspect the indirection block (level=%d)",
	 bcnt + 1 );
#endif
					/* Read the indirect block and..*/
			bp = bread ( 0, dp->de_inode.i_ib[bcnt], 1, SAVEA );
...
#if DEBUG
IOdebug ("	validate_entry :	Note indirect block (level=%d) in bit-map",
	 bcnt + 1 );
#endif
				if ( ref_update )
				{
			...
			{		/* block failed.		*/
#if DEBUG
IOdebug ("	validate_entry :	Clear indirect reference (level=%d)",
	 bcnt + 1 );
#endif
					/* Mark as being freed		*/
...
/*----------------------  General justifications  ----------------------*/
 ...
 {
	print_cur ();
	IOdebug ("%sA different number of (valid) blocks is counted for this entry.", S_WARNING);
	IOdebug ("%s# of blocks counted = %d  |	referred = %d",
		    S_INFO, val_blks, dp->de_inode.i_blocks);
	changes_de++;
	IOdebug ("%sThe number of blocks will be corrected.", S_INFO);
 ...
 {
	print_cur ();
	IOdebug ("%sThe calculated and stored file sizes differ too much.", S_WARNING);
	IOdebug ("%sSize calculated = %d (approx.) |  referred = %d",
		    S_INFO, data_blks * i_fs.fs_bsize, dp->de_inode.i_size);
	IOdebug ("%sThe size field will be corrected.", S_INFO);
...
/*------------------  Interpret a valid direct block  ------------------*/
 ...
 {
#if DEBUG
IOdebug ("	correct_type :	Look for a valid direct block number.");
#endif
	if ( dp->de_inode.i_db[bcnt] != 0 )
	{
		...
		bp = bread ( 0, dp->de_inode.i_db[bcnt], 1, SAVEA );
#if DEBUG
IOdebug ("	correct_type :	Analysis of the content of block no %d",
	 dp->de_inode.i_db[bcnt]);
#endif
					/* A read-error occurred ?	*/
		if ( bp == (struct buf *) NULL )
		{
			IOdebug ("%sRead error occurred", S_SERIOUS);
 ...
 print_cur ();
 if ( dir_flag )
	IOdebug ("%sI interpret the inode as an entry of type DIRECTORY.", S_INFO);
 if ( link_flag )
	IOdebug ("%sI interpret the inode as an entry of type LINK.", S_INFO);

 if ( ! dir_flag && ! link_flag )
	IOdebug ("%sI can't interpret the type of the given entry.", S_INFO);
 ...
 IOdebug (">   The type of the entry is now assumed to be :");
 switch ( dp->de_inode.i_mode )
 {
	case Type_Directory : IOdebug ("	DIRECTORY\n");	break;
	case Type_File	    : IOdebug ("	FILE\n");	break;
	case Type_Link	    : IOdebug ("	LINK\n");	break;
 }
...
#if DEBUG
IOdebug ("	handle_direct :	Examine the content of block no.: %d",
	 ip->i_db[bcnt]);
#endif
	...
	{				/* Could it be a directory-blk?	*/
		case Type_Directory :
		{
			...
			{
				print_cur ();
 IOdebug ("%sDirectory-block on inode slot %d is invalid.", S_SERIOUS, bcnt );
				if ( ! no_corrections )
				{
					wrong_cnt++;
					/* Mark as being freed		*/
		...
		{			/* Only the first direct block	*/
			...
			if ( bcnt == 0 )
			{
				if ( ! possible_link_block ( bp ) )
				{
					print_cur ();
					if ( no_corrections )
 IOdebug ("%sLink-block on inode slot %d is invalid.", S_SERIOUS, bcnt );
 					else
 					{
 IOdebug ("%sInvalid link-block on inode slot %d will be deleted.",
		    S_SERIOUS, bcnt );
						wrong_cnt++;
...
#if DEBUG
IOdebug ("	handle_direct :	  The results : free = %d    wrong = %d",
	 free_cnt, wrong_cnt);
#endif
 ...
 {					/* In advanced (= manual) mode, */
					...
					/* damaged entry.		*/
	print_cur ();
	IOdebug ("%sToo many bad direct blocks kept by the entry! (-> mingood)",
		 S_SERIOUS);
	if ( ! no_corrections ) 
	{
		fst.mingood_dir++;
 ...
 {					/* wrong content.		*/
 	print_cur ();
	IOdebug ("%sThere are a few bad direct blocks kept by the entry !",
		  S_SERIOUS);
	if ( no_corrections )
		return TRUE;
	else
...
handle_single_indirect ( struct buf *bp, word mode, word ref_update )
{
 ...
 {
 	print_cur ();
	IOdebug ("%sToo many invalid block numbers in indirect block found.",
		  S_SERIOUS);
	...
	{
		IOdebug ("%sInvalid single indirect block will be deleted.",
			  S_INFO);
		clr_buf ( bp );
		fst.wrong_ind++;
		test_bwrite ( bp );
       ...
       bcnt < i_fs.fs_maxcontig ; bcnt++ )
 {
	...
	{				/* rectory blocks.		*/
#if DEBUG
IOdebug ("	handle_single_indirect :	Examine dir-block at index-pos %d bnr = %d",
	 bcnt, bpt[bcnt]);
#endif
					/* Read it into memory		*/
		bp_dir = bread ( 0, bpt[bcnt], 1, SAVEA );
		...
		if ( ! possible_dir_block ( bp_dir ) )
		{
		 	print_cur ();
		 	if ( no_corrections )
		 		IOdebug ("%sInvalid directory block kept by indirect block.",
		 			  S_SERIOUS);
		 	else
		 	{
		 		IOdebug ("%sInvalid directory block kept by ind. will be deleted",
		 			  S_SERIOUS);
					/* Mark as being freed		*/
...
#if DEBUG
IOdebug ("	handle_single_indirect : Note at pos %d block number %d",
	 bcnt, bpt[bcnt]);
#endif
					/* A valid dir-block has to be	*/
					/* noted in the ref-maps	*/
...
#if DEBUG
IOdebug ("	handle_double_indirect :	Is it really an indirect block ?");
#endif
					/* Is it an indirect block ?	*/
 if ( ! possible_indirect_block ( bp, TRUE, &changes ) )
 {
 	print_cur ();
	IOdebug ("%sInvalid data in double indirect block found.", S_SERIOUS);
	if ( no_corrections )
		return FALSE;
	else
	{
		IOdebug ("%sInvalid double indirect block will be deleted.",
			  S_INFO);
		clr_buf ( bp );
		fst.wrong_ind++;
		test_bwrite ( bp );
...
#if DEBUG
IOdebug ("	handle_double_indirect : Single indirect block is multiple allocated");
#endif
...
#if DEBUG
IOdebug ("	handle_double_indirect :	Single indirect block at offset %d bnr = %d",
	 bcnt, bpt[bcnt]);
#endif
					/* Read it into memory		*/
	bp_ind = bread ( 0, bpt[bcnt], 1, SAVEA );
	...
	{				/* leted.			*/
	 	...
	 	if ( ! no_corrections )
	 	{
			IOdebug ("%sInvalid single ind. block will be deleted in double ind. block.", S_INFO);
			bitmap_set_free ( bpt[bcnt] );
			bpt[bcnt] = 0;
...
#if DEBUG
IOdebug ("	valid_entry_name :	Test '%32s' for valid entry name",
	 name );
#endif
 ...
 {
	print_cur ();
	IOdebug ("   : Found %d garbage characters in the name buffer\n",
		    ng );
#if 0
	IOdebug ("     [");
	for ( i = 0 ; i < NameMax ; i++ )
...
#if DEBUG
IOdebug ("	valid_pathname :	Test '%40s' for valid pathname",
	 path );
#endif
...
#if DEBUG
IOdebug ("	possible_indirect_block :	Scan block content.");
#endif
					/* Scan all entries		*/
...
#if DEBUG
IOdebug ("	possible_indirect_block :	Errors = %d of %d entries.",
...
#if DEBUG
IOdebug ("	possible_dir_block :	Scan block content.");
#endif
					/* Scan all entries		*/
 for ( i = 0 , valid = 0 , error_cnt = 0 , zero = 0;
...
*/				/* Because of a HFS 1.1 bug, we actu-	*/
...
#if DEBUG
IOdebug (" possible_dir_block :	%d entries : errors= %d  valid= %d  free= %d.",
...
{
#if DEBUG
IOdebug ("	possible_link_block :	Look at link-path '%.40s'",
       bp->b_un.b_link->name );
#endif
...
print_cur ( void )
{
 if ( cur_printed ) 
 	return;
 IOdebug ("%sActual entry = %s : [%s]", S_INFO, cur_path, cur_name);
 cur_printed = TRUE;
}
...
#if DEBUG
IOdebug ("	append_de_hash :	Append '%s' to hash-table on slot : %d",
	 name, index);
#endif
 ...
 {
#if DEBUG
IOdebug ("	append_de_hash :	Unable to allocate memory !");
#endif
	longjmp (term_jmp, 1);
 }
...
#if DEBUG
IOdebug ("	isin_de_hash :	Look for '%s' in hash-table on slot : %d",
	 name, index );
#endif
...
#if DEBUG
IOdebug ("	remove_de_hash :	Clear entry name hash-table.");
#endif
