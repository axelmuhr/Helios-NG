...
#if DEBUG
IOdebug ("	search_entry :	Look for an entry named '%s'", compstr);
#endif
...
#if DEBUG
IOdebug ("	search_entry :	Compare it in block %d at pos. %d with '%s'",
...
#if DEBUG
IOdebug ("	get_next_bnr :	Premature termination of block-scan");
#endif
 ...
 {					/* this procedure		*/
#if DEBUG
IOdebug ("	get_next_bnr :	First time initialization.");
#endif
 ...
 {					
	...
	for ( ; db_cnt < i_fs.fs_ndaddr ; db_cnt++ )
  	{				
#if DEBUG
IOdebug ("	get_next_bnr :	Direct blk : index=%d  bnr=%d",
 ...
 if ( ! ib1_done )			/* If not done so far ...	*/
 {
 	...
 	if ( ! bitmap_get_used ( parent->i_ib[0] ) )
 	{
		...
		{			/* Read the indirect block	*/
			bip1 = bread ( 0, parent->i_ib[0], 1, SAVEA );
		...
		for ( ; ib1_cnt < i_fs.fs_maxcontig ; ib1_cnt++ )
		{
			...
			{
#if DEBUG
IOdebug ("	get_next_bnr :	Single indirect blk : index=%d  bnr=%d",
 ...
 {					/* We only use the double ind.	*/
 	...
 	if ( ! bitmap_get_used ( parent->i_ib[1] ) )
 	{
		...
		{			/* Read the indirect block	*/
			bip2 = bread ( 0, parent->i_ib[1], 1, SAVEA );
		...
		{			/* Read the single indirect	*/
			...
			{		/* Skip over zeroed entries and */
				...
				while ( bip2->b_un.b_daddr[ib2_cnt] == 0 ||
					...
					ib2_cnt++;
#if DEBUG
IOdebug ("	get_next_bnr :	Handle double indirect block numbers. index=%d",
	 ib2_cnt);
#endif
				bip1 = bread ( 0, bip2->b_un.b_daddr[ib2_cnt], 1, SAVEA );
			...
			{			
#if DEBUG
IOdebug ("	get_next_bnr :	Handle double ind. block. Index in single=%d",
	 ib1_cnt);
#endif
...
append_level ( tree_e *elem )
{
 ...
 {
#if DEBUG
IOdebug ("	append_level :	Unable to allocate memory !");
#endif
	longjmp (term_jmp, 1);
 }
...
#if DEBUG
IOdebug ("	append_level :	Add a new subdirectory branch");
#endif
 return tep;
}
...
append_entry ( tree_e *elem )
{
 ...
 {
#if DEBUG
IOdebug ("	append_entry :	Unable to allocate memory !");
#endif
	longjmp (term_jmp, 1);
 }
...
#if DEBUG
IOdebug ("	append_entry :	Add a new subdirectory entry");
#endif
...
#if DEBUG
IOdebug ("	remove_level :	Cut a subdirectory branch");
#endif
...
#if DEBUG
IOdebug ("	remove_entry :	Remove from a subdirectory");
#endif
		   ...
		   char *path )
{
 ...
 {
#if DEBUG
IOdebug ("	append_link_hash :	Unable to allocate memory !");
#endif
 	longjmp (term_jmp, 1);
 }
...
#if DEBUG
IOdebug ("	append_link_hash :	Append '%s' on slot %d",
	  buffer, index);
#endif
...
#if DEBUG
IOdebug ("	takefrom_link_hash :	Look for link-path '%s' (slot %d)",
	  path, index);
#endif
...
#if DEBUG
IOdebug ("	takefrom_link_hash :	The object is referred by %d symbolic links",
	 lnk_cnt );
#endif
...
#if DEBUG
IOdebug ("	remove_link_hash :	Clear symbolic-link hash-table");
#endif
